"""``zaya debug`` — debug tools for Zaya Agent.

Currently supports:
    zaya debug share    Upload debug report (system info + logs) to a
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "zaya-agent/debug-share",
        },
    )
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----ZayaDebugBoundary9f3c"
...
def _resolve_log_path(log_name: str) -> Optional[Path]:
    ...
    """
    from zaya_cli.logs import LOG_FILES
...
def _read_log_tail(log_name: str, num_lines: int) -> str:
    ...
    from zaya_cli.logs import _read_last_n_lines
...
def _capture_dump() -> str:
    """Run ``zaya dump`` and return its stdout as a string."""
...
def collect_debug_report(*, log_lines: int = 200, dump_text: str = "") -> str:
    ...
    dump_text
        Pre-captured dump output.  If empty, ``zaya dump`` is run
        internally.
...
def run_debug(args):
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: zaya debug share [--lines N] [--expire N] [--local]")
        print()
        print("Commands:")
