#!/usr/bin/env python3
"""
Zaya CLI - Main entry point.

Usage:
    zaya                     # Interactive chat (default)
    zaya chat                # Interactive chat
    zaya gateway             # Run gateway in foreground
    zaya gateway start       # Start gateway as service
    zaya gateway stop        # Stop gateway service
    zaya gateway status      # Show gateway status
    zaya gateway install     # Install gateway service
    zaya gateway uninstall   # Uninstall gateway service
    zaya setup               # Interactive setup wizard
    zaya logout              # Clear stored authentication
    zaya status              # Show status of all components
    zaya cron                # Manage cron jobs
    zaya cron list           # List cron jobs
    zaya cron status         # Check if cron scheduler is running
    zaya doctor              # Check configuration and dependencies
    zaya honcho setup                    # Configure Honcho AI memory integration
    zaya honcho status                   # Show Honcho config and connection status
    zaya honcho sessions                 # List directory → session name mappings
    zaya honcho map <name>               # Map current directory to a session name
    zaya honcho peer                     # Show peer names and dialectic settings
    zaya honcho peer --user NAME         # Set user peer name
    zaya honcho peer --ai NAME           # Set AI peer name
    zaya honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    zaya honcho mode                     # Show current memory mode
    zaya honcho mode [hybrid|honcho|local]  # Set memory mode
    zaya honcho tokens                   # Show token budget settings
    zaya honcho tokens --context N       # Set session.context() token cap
    zaya honcho tokens --dialectic N     # Set dialectic result char cap
    zaya honcho identity                 # Show AI peer identity representation
    zaya honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    zaya honcho migrate                  # Step-by-step migration guide: OpenClaw native → Zaya + Honcho
    zaya version             Show version
    zaya update              Update to latest version
    zaya uninstall           Uninstall Zaya Agent
    zaya acp                 Run as an ACP server for editor integration
    zaya sessions browse     Interactive session picker with search

    zaya claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (zaya tools, zaya setup, zaya model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'zaya {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any zaya module import.
#
# Many modules cache ZAYA_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ZAYA_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.zaya/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ZAYA_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the zaya root
    if profile_name is None:
        try:
            from zaya_constants import get_default_zaya_root
            active_path = get_default_zaya_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ZAYA_HOME
    if profile_name is not None:
        try:
            from zaya_cli.profiles import resolve_profile_env
            zaya_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent zaya from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["ZAYA_HOME"] = zaya_home
...
from zaya_cli.config import get_zaya_home
from zaya_cli.env_loader import load_zaya_dotenv
load_zaya_dotenv(project_env=PROJECT_ROOT / '.env')

# Initialize centralized file logging early — all `zaya` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from zaya_logging import setup_logging as _setup_logging
...
try:
    from zaya_cli.config import load_config as _load_config_early
    from zaya_constants import apply_ipv4_preference as _apply_ipv4
...
from zaya_cli import __version__, __release_date__
from zaya_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from zaya_cli.config import get_env_path, get_zaya_home, load_config
    from zaya_cli.auth import get_auth_status

    # Determine whether Zaya itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from zaya_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_zaya_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from zaya_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_zaya_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Zaya has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Zaya to use their tokens.
    if _has_zaya_config:
        try:
...
def _resolve_last_cli_session() -> Optional[str]:
    ...
    try:
        from zaya_state import SessionDB
        db = SessionDB()
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, zaya_bin
        cli_args: the original CLI arguments (everything after 'zaya')
    """
    import shutil
    ...
    zaya_bin = container_info["zaya_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f'  }}];\n'
                    f"\n"
                    f"Or run: sudo zaya {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo zaya {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'zaya sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Zaya isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  zaya setup")
        print()

        from zaya_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'zaya setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from zaya_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ZAYA_YOLO_MODE"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ZAYA_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from zaya_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from zaya_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Zaya?")
        print()
    ...
    session_dir = get_zaya_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: zaya gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  zaya gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Zaya Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  zaya gateway")
        ...
        print()
        print("  Or install as a service: zaya gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'zaya whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from zaya_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``zaya model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from zaya_cli.auth import (
        resolve_provider, AuthError, format_auth_error,
    )
    from zaya_cli.config import get_compatible_custom_providers, load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("ZAYA_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
    ...
    from zaya_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.zaya/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.zaya/.env if the active provider is not 'custom'.
    ...
    """
    from zaya_cli.config import get_env_value, save_env_value, load_config
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from zaya_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from zaya_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from zaya_cli.config import get_env_value, save_env_value
    ...
    from zaya_cli.models import model_ids, get_pricing_for_provider
    ...
    if selected:
        ...
        from zaya_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from zaya_cli.config import get_env_value, save_config, save_env_value
    from zaya_cli.nous_subscription import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from zaya_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from zaya_cli.codex_models import get_codex_model_ids
    import argparse
    ...
    _codex_token = None
    # Prefer credential pool (where `zaya auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from zaya_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from zaya_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from zaya_cli.auth import _save_model_choice, deactivate_provider
    from zaya_cli.config import get_env_value, load_config, save_config
    ...
    from zaya_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `zaya model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from zaya_cli.curses_ui import flush_stdin
        flush_stdin()
        print()
...
from zaya_cli.models import _PROVIDER_MODELS
...
def _model_flow_copilot(config, current_model=""):
    ...
    from zaya_cli.config import save_env_value, load_config, save_config
    from zaya_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from zaya_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from zaya_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Zaya turns to `copilot --acp`.")
    print("  Zaya currently starts its own ACP subprocess for each request.")
    print("  Zaya uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set ZAYA_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from zaya_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from zaya_cli.config import get_env_value, save_env_value, load_config, save_config
    from zaya_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from zaya_constants import display_zaya_home as _dhh_fn
            print(f"    Zaya will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from zaya_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'zaya model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Zaya CLI with a provider."""
    from zaya_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from zaya_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from zaya_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from zaya_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from zaya_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from zaya_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from zaya_cli.doctor import run_doctor
    run_doctor(args)


def cmd_dump(args):
    """Dump setup summary for support/debugging."""
    from zaya_cli.dump import run_dump
    run_dump(args)


def cmd_debug(args):
    """Debug tools (share report, etc.)."""
    from zaya_cli.debug import run_debug
    run_debug(args)


def cmd_config(args):
    """Configuration management."""
    from zaya_cli.config import config_command
    config_command(args)


def cmd_backup(args):
    """Back up Zaya home directory to a zip file."""
    if getattr(args, "quick", False):
        from zaya_cli.backup import run_quick_backup
        run_quick_backup(args)
    else:
        from zaya_cli.backup import run_backup
        run_backup(args)


def cmd_import(args):
    """Restore a Zaya backup from a zip file."""
    from zaya_cli.backup import run_import
    run_import(args)


def cmd_version(args):
    """Show version."""
    print(f"Zaya Agent v{__version__} ({__release_date__})")
    ...
    try:
        from zaya_cli.banner import check_for_updates
        from zaya_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Zaya Agent."""
    _require_tty("uninstall")
    from zaya_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``zaya update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``zaya web``).
    ...
    if r1.returncode != 0:
        print(f"  {'✗' if fatal else '⚠'} Web UI npm install failed"
              + ("" if fatal else " (zaya web will not be available)"))
        if fatal:
...
def _update_via_zip(args):
    """Update Zaya Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/NousResearch/zaya-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="zaya-update-")
        zip_path = os.path.join(tmp_dir, f"zaya-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to zaya-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"zaya-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("zaya-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Zaya behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes zaya completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Zaya couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Zaya couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Zaya behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `zaya update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/zaya-agent.git",
    "git@github.com:NousResearch/zaya-agent.git",
    "https://github.com/NousResearch/zaya-agent",
    "git@github.com:NousResearch/zaya-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/zaya-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from zaya_constants import get_zaya_home
    return (get_zaya_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from zaya_constants import get_zaya_home
        (get_zaya_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Zaya repository.")
        print("  This means you may miss updates from NousResearch/zaya-agent.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/NousResearch/zaya-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/NousResearch/zaya-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``zaya update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from zaya_constants import get_default_zaya_root
    default_home = get_default_zaya_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "zaya-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Zaya Agent to the latest version."""
    import shutil
    from zaya_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Zaya Agent")
        return
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/NousResearch/zaya-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_zaya_home added to zaya_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload zaya_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_zaya_home() added since the last release.
        try:
            import importlib
            import zaya_constants as _hc
        ...
        try:
            from zaya_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'zaya config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: zaya config migrate")
            else:
                print()
                print("Skipped. Run 'zaya config migrate' later to configure.")
        else:
        ...
        # When running as ``zaya update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart zaya-gateway`` kills everything in the
        ...
        if gateway_mode:
            _exit_code_path = get_zaya_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all zaya-gateway* units (default + profiles)
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "zaya-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. zaya-gateway.service or zaya-gateway-coder.service
            ...
            if is_macos():
                try:
                    from zaya_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: zaya gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: zaya -p <profile> gateway run  for each profile)")
        ...
        print("  zaya model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``zaya -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `zaya profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_zaya_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → zaya -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.zaya)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  zaya profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     zaya -p {name} chat")
                else:
        ...
        name = args.profile_name
        from zaya_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from zaya_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from zaya_cli.profiles import rename_profile
        try:
        ...
        from zaya_cli.profiles import export_profile
        name = args.profile_name
        ...
        from zaya_cli.profiles import import_profile
        try:
...
def cmd_dashboard(args):
    ...
    try:
        ...
        print("Install them with:  pip install zaya-agent[web]")
        sys.exit(1)

    if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
        sys.exit(1)

    from zaya_cli.web_server import start_server
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from zaya_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Zaya log files."""
    from zaya_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for zaya CLI."""
    parser = argparse.ArgumentParser(
        prog="zaya",
        description="Zaya Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    zaya                        Start interactive chat
    zaya chat -q "Hello"        Single query mode
    zaya -c                     Resume the most recent session
    zaya -c "my project"        Resume a session by name (latest in lineage)
    zaya --resume <session_id>  Resume a specific session by ID
    zaya setup                  Run setup wizard
    zaya logout                 Clear stored authentication
    zaya auth add <provider>    Add a pooled credential
    zaya auth list              List pooled credentials
    zaya auth remove <p> <t>    Remove pooled credential by index, id, or label
    zaya auth reset <provider>  Clear exhaustion status for a provider
    zaya model                  Select default model
    zaya config                 View configuration
    zaya config edit            Edit config in $EDITOR
    zaya config set model gpt-4 Set a config value
    zaya gateway                Run messaging gateway
    zaya -s zaya-agent-dev,github-auth
    zaya -w                     Start in isolated git worktree
    zaya gateway install        Install gateway background service
    zaya sessions list          List past sessions
    zaya sessions browse        Interactive session picker
    zaya sessions rename ID T   Rename/title a session
    zaya logs                   View agent.log (last 50 lines)
    zaya logs -f                Follow agent.log in real time
    zaya logs errors            View errors.log
    zaya logs --since 1h        Lines from the last hour
    zaya debug share             Upload debug report for support
    zaya update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Zaya Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: zaya-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Zaya Agent with an interactive wizard. "
                    "Run a specific section: zaya setup model|tts|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Zaya CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: zaya-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Zaya Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Zaya Agent setup"
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Zaya setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Zaya Agent. Use 'zaya debug share' to "
                    "upload a debug report (system info + recent logs) to a paste "
                    "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    zaya debug share              Upload debug report and print URL
    zaya debug share --lines 500  Include more log lines
    zaya debug share --expire 30  Keep paste for 30 days
    zaya debug share --local      Print report locally (no upload)
""",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Zaya home directory to a zip file",
        description="Create a zip archive of your entire Zaya configuration, "
                    "skills, sessions, and data (excludes the zaya-agent codebase). "
                    "Use --quick for a fast snapshot of just critical state files."
    )
    backup_parser.add_argument(
        "-o", "--output",
        help="Output path for the zip file (default: ~/zaya-backup-<timestamp>.zip)"
    )
    backup_parser.add_argument(
        "-q", "--quick",
        action="store_true",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Zaya backup from a zip file",
        description="Extract a previously created Zaya backup into your "
                    "Zaya home directory, restoring configuration, skills, "
                    "sessions, and data"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Zaya Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from zaya_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from zaya_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from zaya_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/zaya-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from zaya_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from zaya_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'zaya tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # zaya tools disable <name...> [--platform cli]
    ...
    # zaya tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from zaya_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from zaya_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Zaya as an MCP server",
        description=(
            "Manage MCP server connections and run Zaya as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'zaya mcp add' to connect to a new server, or\n"
            "'zaya mcp serve' to expose Zaya conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Zaya as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from zaya_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch zaya --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            zaya_bin = shutil.which("zaya")
            if zaya_bin:
                os.execvp(zaya_bin, ["zaya", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "zaya_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Zaya"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Zaya",
    ...
    def cmd_claw(args):
        from zaya_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Zaya Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Zaya Agent",
        description="Remove Zaya Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Zaya Agent as an ACP (Agent Client Protocol) server",
        description="Start Zaya Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Zaya Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Zaya instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Zaya Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Zaya log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    zaya logs                    Show last 50 lines of agent.log
    zaya logs -f                 Follow agent.log in real time
    zaya logs errors             Show last 50 lines of errors.log
    zaya logs gateway -n 100     Show last 100 lines of gateway.log
    zaya logs --level WARNING    Only show WARNING and above
    zaya logs --session abc123   Filter by session ID
    zaya logs --component tools  Only show tool-related lines
    zaya logs --since 1h         Lines from the last hour
    zaya logs --since 30m -f     Follow, starting from 30 min ago
    zaya logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``zaya -c Pokemon Agent Dev`` → ``zaya -c 'Pokemon Agent Dev'``
    ...
    from zaya_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'zaya -c model' where 'model' is consumed as the
