...
- ``zaya_cli.providers``        -- canonical provider identity + overlays
- ``zaya_cli.model_normalize``  -- per-provider name formatting
...
from zaya_cli.providers import (
...
_ZAYA_MODEL_WARNING = (
    "Nous Research Zaya 3 & 4 models are NOT agentic and are not designed "
    "for use with Zaya Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Zaya 3 / Zaya 4 chat families.
# The previous substring check (`"zaya" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``zaya-brain:qwen3-14b-ctx16k`` that just
# happen to carry "zaya" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Zaya-3-Llama-3.1-70B, zaya-4-405b, openrouter/zaya3:70b
# Negative examples it must NOT match:
#   zaya-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_ZAYA_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])zaya[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_zaya_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Zaya 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_ZAYA_NON_AGENTIC_RE.search(model_name))


def _check_zaya_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Zaya 3/4 chat model."""
    if is_nous_zaya_non_agentic(model_name):
        return _ZAYA_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'zaya model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'zaya doctor' — config issues detected:"
    ...
    zaya_warn = _check_zaya_model_warning(new_model)
    if zaya_warn:
        warnings.append(zaya_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from zaya_cli/models.py (OPENROUTER_MODELS,
    ...
    from zaya_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by zaya provider ID
    ...
    # --- 1. Check Zaya-mapped providers ---
    for zaya_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(zaya_id)
    ...
    # --- 2. Check Zaya-only providers (nous, openai-codex, copilot, opencode-go) ---
    from zaya_cli.providers import ZAYA_OVERLAYS
    from zaya_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Zaya provider ID.
    # ZAYA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Zaya IDs ("copilot").
    _mdev_to_zaya = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ZAYA_OVERLAYS.items():
        if pid in seen_slugs:
            continue

        # Resolve Zaya slug — e.g. "github-copilot" → "copilot"
        zaya_slug = _mdev_to_zaya.get(pid, pid)
        if zaya_slug in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                from zaya_cli.auth import _load_auth_store
                store = _load_auth_store()
                providers_store = store.get("providers", {})
                pool_store = store.get("credential_pool", {})
                if store and (
                    pid in providers_store or zaya_slug in providers_store
                    or pid in pool_store or zaya_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(zaya_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", zaya_slug, exc)
        ...
        # configured.
        if not has_creds and zaya_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_zaya_oauth_credentials,
                )
                zaya_creds = read_zaya_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (zaya_creds and zaya_creds.get("accessToken")) or \
        ...
        # Use curated list — look up by Zaya slug, fall back to overlay key
        model_ids = curated.get(zaya_slug, []) or curated.get(pid, [])
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": zaya_slug,
            "name": get_label(zaya_slug),
            "is_current": zaya_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "zaya",
        ...
        seen_slugs.add(pid)
        seen_slugs.add(zaya_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ZAYA_OVERLAYS (keeps /model in sync
    # with `zaya model`).
    try:
        from zaya_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
