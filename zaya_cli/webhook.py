"""zaya webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.zaya/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _zaya_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: zaya gateway run
"""
...
def webhook_command(args):
    """Entry point for 'zaya webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: zaya webhook {subscribe|list|remove|test}")
        print("Run 'zaya webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (zaya gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: zaya webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from zaya webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (zaya gateway run)")
