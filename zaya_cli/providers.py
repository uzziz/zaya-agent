"""
Single source of truth for provider identity in Zaya Agent.
...
2. **Zaya overlays** — transport type, auth patterns, aggregator flags,
...
# -- Zaya overlay ----------------------------------------------------------
# Zaya-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ZayaOverlay:
    """Zaya-specific provider metadata layered on top of models.dev."""
...
ZAYA_OVERLAYS: Dict[str, ZayaOverlay] = {
    "openrouter": ZayaOverlay(
    ...
    "openai-codex": ZayaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": ZayaOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ZAYA_QWEN_BASE_URL",
    ),
    "copilot-acp": ZayaOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ZayaOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ZayaOverlay(
    ...
    "minimax-cn": ZayaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ZayaOverlay(
        transport="openai_chat",
    ...
    "opencode": ZayaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ZayaOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "zaya", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Zaya overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Zaya overlay
    ...
    overlay = ZAYA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + zaya extra
    ...
    if overlay is not None:
        # Zaya-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
