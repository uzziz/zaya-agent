...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_zaya_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:zaya_pkce",
    ...
    raise SystemExit(f"`zaya auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    if removed.source.startswith("env:"):
        ...
        if env_var:
            from zaya_cli.config import remove_env_value
            cleared = remove_env_value(env_var)
            if cleared:
                print(f"Cleared {env_var} from .env")

    # If this was a singleton-seeded credential (OAuth device_code, zaya_pkce),
    ...
    # re-seeded on the next load_pool() call.
    ...
    elif removed.source == "zaya_pkce" and provider == "anthropic":
        from zaya_constants import get_zaya_home
        oauth_file = get_zaya_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared Zaya Anthropic OAuth credentials")

    elif removed.source == "claude_code" and provider == "anthropic":
        from zaya_cli.auth import suppress_credential_source
        ...
        print("Run `zaya auth add anthropic` to re-enable if needed.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `zaya auth` is called bare."""
