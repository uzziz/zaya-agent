"""
Configuration management for Zaya Agent.

Config files are stored in ~/.zaya/ for easy access:
- ~/.zaya/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.zaya/.env         - API keys and secrets

This module provides:
- zaya config          - Show current configuration
- zaya config edit     - Open config in editor
- zaya config set      - Set a specific value
- zaya config wizard   - Re-run setup wizard
"""
...
from zaya_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ZAYA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_zaya_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Zaya is running in package-manager-managed mode.

    Two signals: the ZAYA_MANAGED env var (set by the systemd service),
    or a .managed marker file in ZAYA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade zaya-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "zaya update"


def format_managed_message(action: str = "modify this Zaya installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ZAYA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Zaya installation is managed by NixOS "
            f"(ZAYA_MANAGED={env_hint}).\n"
            "Edit services.zaya-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Zaya installation is managed by Homebrew "
            f"(ZAYA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade zaya-agent"
        )

    return (
        f"Cannot {action}: this Zaya installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Zaya."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ZAYA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, zaya_bin
    or None if container mode is not active, we're already inside the
    container, or ZAYA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ZAYA_DEV") == "1":
        return None
    ...
    container_mode_file = get_zaya_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "zaya-agent")
    exec_user = info.get("exec_user", "zaya")
    zaya_bin = info.get("zaya_bin", "/data/current-package/bin/zaya")
...
# Re-export from zaya_constants — canonical definition lives there.
from zaya_constants import get_zaya_home  # noqa: F811,E402
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the zaya group can
    share state with the gateway service.

    The mode can be overridden via the ZAYA_HOME_MODE environment variable
    (e.g. ZAYA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ZAYA_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ZAYA_HOME_MODE", "").strip()
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ZAYA_HOME if the user doesn't have one yet."""
...
def ensure_zaya_home():
    """Ensure ~/.zaya directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_zaya_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_zaya_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ZAYA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when Zaya
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Zaya sends a stable profile-scoped userId to Camofox
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.zaya/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.zaya/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for zaya-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Zaya Agent*" header.
    ...
    # Logging — controls file logging to ~/.zaya/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Zaya auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @zaya:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ZAYA_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'zaya-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Zaya API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Zaya API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Zaya API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ZAYA_TOOL_PROGRESS and ZAYA_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.zaya.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'zaya setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Zaya won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'zaya doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ZAYA_TOOL_PROGRESS")
            old_mode = get_env_value("ZAYA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ZAYA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ZAYA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ZAYA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ZAYA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: zaya config set <key> <value>")
    ...
    # metadata.zaya.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.zaya/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.zaya/config.yaml."""
    import copy
    ensure_zaya_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — zaya auth) — OpenAI Codex
#   nous         (OAuth — zaya auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.zaya/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.zaya/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Zaya env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.zaya/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.zaya/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.zaya/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.zaya/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Zaya — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Zaya vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.zaya/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Zaya Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  zaya config edit     # Edit config file", Colors.DIM))
    print(color("  zaya config set <key> <value>", Colors.DIM))
    print(color("  zaya setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: zaya config set <key> <value>")
            print()
            print("Examples:")
            print("  zaya config set model anthropic/claude-sonnet-4")
            print("  zaya config set terminal.backend docker")
            print("  zaya config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'zaya config migrate' to add them")
        ...
        print("  zaya config           Show current configuration")
        print("  zaya config edit      Open config in editor")
        print("  zaya config set <key> <value>   Set a config value")
        print("  zaya config check     Check for missing/outdated config")
        print("  zaya config migrate   Update config with new options")
        print("  zaya config path      Show config file path")
        print("  zaya config env-path  Show .env file path")
        sys.exit(1)
