"""
Status command for zaya CLI.

Shows the status of all Zaya Agent components.
"""
...
from zaya_cli.auth import AuthError, resolve_provider
from zaya_cli.colors import Colors, color
from zaya_cli.config import get_env_path, get_env_value, get_zaya_home, load_config
from zaya_cli.models import provider_label
from zaya_cli.nous_subscription import get_nous_subscription_features
from zaya_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all Zaya Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Zaya Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from zaya_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: zaya model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: zaya model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: zaya model)"
    ...
    if not terminal_env:
        ...
        # (zaya status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    if _is_termux():
        try:
            from zaya_cli.gateway import find_gateway_pids
        ...
        from zaya_cli.gateway import get_launchd_label
        try:
            result = subprocess.run(
    ...
    jobs_file = get_zaya_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_zaya_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'zaya doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'zaya setup' to configure", Colors.DIM))
    print()
