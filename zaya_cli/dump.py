"""
Dump command for zaya CLI.

Outputs a compact, plain-text summary of the user's Zaya setup
...
from zaya_cli.config import get_zaya_home, get_env_path, get_project_root, load_config
...
def _count_skills(zaya_home: Path) -> int:
...
def _cron_summary(zaya_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = zaya_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"zaya_home:      {display_zaya_home()}")
    ...
    toolsets = config.get("toolsets", ["zaya-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(zaya_home)}")
    lines.append(f"  skills:             {_count_skills(zaya_home)}")
