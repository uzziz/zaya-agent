"""zaya claw — OpenClaw migration commands.
...
Usage:
    ...
    zaya claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    zaya claw cleanup              # Archive leftover OpenClaw directories
    zaya claw cleanup --dry-run    # Preview what would be archived
"""
...
from zaya_cli.config import get_zaya_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Zaya may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Zaya gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Zaya gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'zaya stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_zaya.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_zaya", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: zaya claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Zaya")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'zaya claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Zaya migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: zaya claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: zaya claw migrate --yes")
            return
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: zaya claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: zaya claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  zaya claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  zaya claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  zaya config set OPENROUTER_API_KEY sk-or-v1-...")
