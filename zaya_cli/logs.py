"""``zaya logs`` — view and filter Zaya log files.
...
Usage examples::
    ...
    zaya logs                    # last 50 lines of agent.log
    zaya logs -f                 # follow agent.log in real time
    zaya logs errors             # last 50 lines of errors.log
    zaya logs gateway -n 100    # last 100 lines of gateway.log
    zaya logs --level WARNING    # only WARNING+ lines
    zaya logs --session abc123   # filter by session ID substring
    zaya logs --component tools  # only tool-related lines
    zaya logs --since 1h         # lines from the last hour
    zaya logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_zaya_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Zaya runs — try 'zaya chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_zaya_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_zaya_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_zaya_home()}/logs/")
        return

    print(f"Log files in {display_zaya_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'zaya chat' to generate logs)")
