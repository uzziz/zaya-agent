...
from zaya_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: zaya setup tools, zaya setup terminal, or zaya status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `zaya setup` and `zaya model`."""
    if not managed_nous_tools_enabled():
        return set()
