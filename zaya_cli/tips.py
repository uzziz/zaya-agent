...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or zaya -c.",
    ...
    "Ctrl+Z suspends Zaya to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "zaya -c resumes your most recent CLI session. zaya -c \"project name\" resumes by title.",
    "zaya -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "zaya -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "zaya chat -t web,terminal enables only specific toolsets for a focused session.",
    "zaya chat -s github-pr-workflow preloads a skill at launch.",
    "zaya chat -q \"query\" runs a single non-interactive query and exits.",
    "zaya chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "zaya chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "zaya --yolo bypasses all dangerous command approval prompts for the entire session.",
    "zaya chat --source telegram tags the session for filtering in zaya sessions list.",
    "zaya -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "zaya doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "zaya dump outputs a compact setup summary — great for bug reports.",
    "zaya config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "zaya config edit opens config.yaml in your default editor.",
    "zaya config check scans for missing or stale configuration options.",
    "zaya sessions browse opens an interactive session picker with search.",
    "zaya sessions stats shows session counts by platform and database size.",
    "zaya sessions prune --older-than 30 cleans up old sessions.",
    "zaya skills search react --source skills-sh searches the skills.sh public directory.",
    "zaya skills check scans installed hub skills for upstream updates.",
    "zaya skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "zaya skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "zaya mcp add github --command npx adds MCP servers from the command line.",
    "zaya mcp serve runs Zaya itself as an MCP server for other agents.",
    "zaya auth add lets you add multiple API keys for credential pool rotation.",
    "zaya completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "zaya logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "zaya backup creates a zip backup of your entire Zaya home directory.",
    "zaya profile create coder creates an isolated profile that becomes its own command.",
    "zaya profile create work --clone copies your current config and keys to a new profile.",
    "zaya update syncs new bundled skills to ALL profiles automatically.",
    "zaya gateway install sets up Zaya as a system service (systemd/launchd).",
    "zaya memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "zaya webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as zaya -w).",
    ...
    "ZAYA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Zaya tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'zaya profile create coder' creates the 'coder' command.",
    "zaya profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Zaya prints a resume command with session ID and stats.",
    "zaya sessions export backup.jsonl exports all sessions for backup or analysis.",
    "zaya -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "zaya skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .zaya/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: zaya cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Zaya runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "zaya gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Zaya via a local macOS server.",
    ...
    "Context files (.zaya.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.zaya/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Zaya loads project context from .zaya.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — zaya tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.zaya/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "zaya chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "zaya plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "zaya acp runs Zaya as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ZAYA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "zaya login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "zaya status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.zaya/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.zaya/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Zaya your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.zaya/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Zaya auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.zaya/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at ZAYA_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "ZAYA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ZAYA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.zaya/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'zaya profile create ls' would be rejected.",
    "zaya profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "zaya claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ZAYA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via zaya tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.zaya/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.zaya/skills/.hub/quarantine/ holds skills pending security review.",
]
