"""
Profile management for multiple isolated Zaya instances.

Each profile is a fully independent ZAYA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.zaya/profiles/<name>/`` by default.

The "default" profile is ``~/.zaya`` itself — backward compatible,
zero migration needed.

Usage::

    zaya profile create coder          # fresh profile + bundled skills
    zaya profile create coder --clone  # also copy config, .env, SOUL.md
    zaya profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    zaya -p coder chat                 # or via flag
    zaya profile use coder             # set as sticky default
    zaya profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See zaya_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.zaya) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "zaya-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "zaya", "default", "test", "tmp", "root", "sudo",
})

# Zaya subcommands that cannot be used as profile names/aliases
_ZAYA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the zaya root, NOT to the current ZAYA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ZAYA_HOME points outside
    ``~/.zaya``, profiles live under ``ZAYA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_zaya_home() / "profiles"


def _get_default_zaya_home() -> Path:
    """Return the default (pre-profile) ZAYA_HOME path.

    In standard deployments this is ``~/.zaya``.
    In Docker/custom deployments where ZAYA_HOME is outside ``~/.zaya``
    (e.g. ``/opt/data``), returns ZAYA_HOME directly.
    """
    from zaya_constants import get_default_zaya_root
    return get_default_zaya_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_zaya_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ZAYA_HOME directory."""
    if name == "default":
        return _get_default_zaya_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, zaya subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _ZAYA_SUBCOMMANDS:
        return f"'{name}' conflicts with a zaya subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_zaya_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.zaya)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ZAYA_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ZAYA_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.zaya).\n"
            "To remove everything, use: zaya uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ZAYA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ZAYA_HOME")
    try:
        os.environ["ZAYA_HOME"] = str(profile_dir)
        from zaya_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ZAYA_HOME"] = old_home
        elif "ZAYA_HOME" in os.environ:
            del os.environ["ZAYA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.zaya/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: zaya profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ZAYA_HOME.

    Returns ``"default"`` if ZAYA_HOME is not set or points to ``~/.zaya``.
    Returns the profile name if ZAYA_HOME points into ``~/.zaya/profiles/<name>``.
    Returns ``"custom"`` if ZAYA_HOME is set to an unrecognized path.
    """
    from zaya_constants import get_zaya_home
    zaya_home = get_zaya_home()
    resolved = zaya_home.resolve()

    default_resolved = _get_default_zaya_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.zaya itself — its parent is ~/ and its
        # directory name is ".zaya", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: zaya profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.zaya itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.zaya). "
            "Specify a different name: zaya profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for zaya profile names."""
    return '''# Zaya Agent profile completion
# Add to ~/.bashrc: eval "$(zaya completion bash)"

_zaya_profiles() {
    local profiles_dir="$HOME/.zaya/profiles"
...
_zaya_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_zaya_profiles)" -- "$cur"))
        return
    fi
...
complete -F _zaya_completion zaya
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for zaya profile names."""
    return '''#compdef zaya
# Zaya Agent profile completion
# Add to ~/.zshrc: eval "$(zaya completion zsh)"

_zaya() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.zaya/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.zaya/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ZAYA_HOME path string.

    Called early in the CLI entry point, before any zaya modules
    are imported, to set the ZAYA_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: zaya profile create {profile_name}"
        )
