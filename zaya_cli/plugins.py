...
1. **User plugins**   – ``~/.zaya/plugins/<name>/``
2. **Project plugins** – ``./.zaya/plugins/<name>/`` (opt-in via
   ``ZAYA_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``zaya_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "zaya_agent.plugins"

_NS_PARENT = "zaya_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``zaya honcho ...``).
    ...
    def register_skill(
        ...
        ``~/.zaya/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.zaya/plugins/)
        user_dir = get_zaya_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.zaya/plugins/)
        if _env_enabled("ZAYA_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".zaya" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``zaya_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``zaya tools`` TUI so plugin-provided toolsets appear
