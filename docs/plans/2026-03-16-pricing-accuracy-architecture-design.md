...
Zaya should only show dollar costs when they are backed by an official source for the user's actual billing path.
...
Current Zaya behavior has four structural issues:
...
Zaya must stop keying pricing solely by `model`.
...
- manual `zaya pricing sync`

## Reconciliation Architecture

Live requests may produce only an estimate initially. Zaya should reconcile them later when a provider exposes actual billed cost.

Suggested flow:

1. Agent call completes.
2. Zaya stores canonical usage plus reconciliation ids.
3. Zaya computes an immediate estimate if a pricing source exists.
...
That path cannot satisfy the product requirement. Zaya should instead migrate to:
...
This is the minimum architecture that makes the statement "Zaya pricing is backed by official sources where possible, and otherwise clearly labeled" defensible.
