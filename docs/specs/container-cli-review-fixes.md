...
**PR:** NousResearch/zaya-agent#7543
...
1. **Let it crash.** No silent fallbacks. If `.container-mode` exists but something goes wrong, the error propagates naturally (Python traceback). The only case where container routing is skipped is when `.container-mode` doesn't exist or `ZAYA_DEV=1`.
...
1. get_container_exec_info()
   - ZAYA_DEV=1 → return None (skip routing)
...
2. _exec_in_container(container_info, sys.argv[1:])
   ...
   c. Build exec_cmd: [sudo? + runtime, "exec", tty_flags, "-u", exec_user, env_flags, container, zaya_bin, *cli_args]
...
Change: Remove the try/except around file reading. Keep the early returns for `ZAYA_DEV=1` and `_is_inside_container()`. The `FileNotFoundError` from `open()` when `.container-mode` doesn't exist should still return `None` (this is the "container mode not enabled" case). All other exceptions propagate.
...
if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
  ...
  echo "zaya-agent: backing up existing ${symlinkPath} to $_backup"
...
Note: there is a theoretical race between the `[ -d ... ]` check and the `mv` (something could create/remove the directory in between). In practice this is a NixOS activation script running as root during `nixos-rebuild switch` — no other process should be touching `~/.zaya` at that moment. Not worth adding locking for.
...
The existing test file (`tests/zaya_cli/test_container_aware_cli.py`) has 16 tests. With the simplified exec model, several are obsolete.
...
- `test_get_container_exec_info_skipped_when_zaya_dev` — unchanged
- `test_get_container_exec_info_not_skipped_when_zaya_dev_zero` — unchanged
...
- Changes to the `ZAYA_DEV=1` bypass
