...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs ZayaAgentLoop with terminal tool only
...
Requires: pip install zaya-agent[yc-bench]
"""
...
class YCBenchEvalConfig(ZayaAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends ZayaAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(ZayaAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The ZayaAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs ZayaAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our ZayaAgentLoop.
