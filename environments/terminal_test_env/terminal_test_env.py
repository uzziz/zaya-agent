...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Zaya Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Zaya Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Zaya Agent",
    },
    {
...
class TerminalTestEnv(ZayaAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/zaya-agent/.env.
        """
