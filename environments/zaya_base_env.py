"""
ZayaAgentBaseEnv -- Abstract Base Environment for Zaya-Agent + Atropos

Provides the Atropos integration plumbing that all zaya-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via ZayaAgentLoop
...
# Ensure the zaya-agent repo root is on sys.path so that imports like
...
# Load API keys from zaya-agent/.env so all environments can access them
...
# Import zaya-agent toolset infrastructure
...
class ZayaAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for zaya-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of zaya toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: zaya, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/zaya-results/ "
...
class ZayaAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for zaya-agent Atropos integration.
    ...
    name: Optional[str] = "zaya-agent"
    env_config_cls = ZayaAgentEnvConfig

    def __init__(
        self,
        config: ZayaAgentEnvConfig,
        ...
        # Set terminal environment variables so zaya tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY zaya-agent tool (terminal, file, web,
