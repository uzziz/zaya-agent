"""
Zaya MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Zaya-specific extra)

Usage:
    zaya mcp serve
    zaya mcp serve --verbose
...
logger = logging.getLogger("zaya.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ZAYA_HOME."""
    try:
        from zaya_constants import get_zaya_home
        return get_zaya_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ZAYA_HOME", Path.home() / ".zaya")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from zaya_constants import get_zaya_home
        directory_file = get_zaya_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ZAYA_HOME", Path.home() / ".zaya")
...
class EventBridge:
    ...
    This is the Zaya equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from zaya_constants import get_zaya_home
            db_file = get_zaya_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ZAYA_HOME", Path.home() / ".zaya")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Zaya MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'zaya-agent[mcp]'"
        )

    mcp = FastMCP(
        "zaya",
        instructions=(
            "Zaya Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Zaya MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
