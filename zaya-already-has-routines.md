# Zaya Agent Has Had "Routines" Since March
...
Zaya equivalent — works today:
```bash
zaya webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Zaya implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Zaya Agent |
|---|---|---|
...
## Things Zaya Does That Routines Can't
...
Zaya has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Zaya Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
pip install zaya-agent
zaya setup
```
...
```bash
zaya gateway setup    # enable webhooks
zaya webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [zaya-agent.nousresearch.com/docs/guides/automation-templates](https://zaya-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [zaya-agent.nousresearch.com](https://zaya-agent.nousresearch.com)

GitHub: [github.com/NousResearch/zaya-agent](https://github.com/NousResearch/zaya-agent)

---

*Zaya Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
