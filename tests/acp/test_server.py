"""Tests for acp_adapter.server — ZayaACPAgent ACP server."""
...
from acp_adapter.server import ZayaACPAgent, ZAYA_VERSION
...
def agent(mock_manager):
    """ZayaACPAgent backed by a mock session manager."""
    return ZayaACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "zaya-agent"
        assert resp.agent_info.version == ZAYA_VERSION
...
class TestSlashCommands:
    ...
    def test_model_switch_uses_requested_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("zaya_cli.config.load_config", lambda: {
        ...
        with patch("run_agent.AIAgent", side_effect=fake_agent):
            acp_agent = ZayaACPAgent(session_manager=manager)
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["zaya-acp"]
