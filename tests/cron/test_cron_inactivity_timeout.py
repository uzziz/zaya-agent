...
- Unlimited timeout (ZAYA_CRON_TIMEOUT=0)
- Backward compat: ZAYA_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """ZAYA_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """ZAYA_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("ZAYA_CRON_TIMEOUT", "1200")
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """ZAYA_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("ZAYA_CRON_TIMEOUT", "0")
        _cron_timeout = float(os.getenv("ZAYA_CRON_TIMEOUT", 600))
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
...
class TestSysPathOrdering:
    ...
    def test_zaya_time_importable(self):
        """zaya_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _zaya_now
        assert callable(_zaya_now)

    def test_zaya_constants_importable(self):
        """zaya_constants should be importable from cron context."""
        from zaya_constants import get_zaya_home
        assert callable(get_zaya_home)
