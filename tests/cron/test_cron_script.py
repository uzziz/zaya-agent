...
- Path resolution (absolute, relative to ZAYA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ZAYA_HOME."""
    zaya_home = tmp_path / ".zaya"
    zaya_home.mkdir()
    (zaya_home / "cron").mkdir()
    (zaya_home / "cron" / "output").mkdir()
    (zaya_home / "scripts").mkdir()
    monkeypatch.setenv("ZAYA_HOME", str(zaya_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ZAYA_DIR", zaya_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", zaya_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", zaya_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", zaya_home / "cron" / "output")
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("ZAYA_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.zaya/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ZAYA_SESSION_PLATFORM") is None
        assert os.environ.get("ZAYA_SESSION_CHAT_ID") is None
        assert os.environ.get("ZAYA_SESSION_CHAT_NAME") is None
