...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("zaya_cli.config.get_config_path", return_value=config_path), \
             patch("zaya_cli.config.ensure_zaya_home"):
    ...
    def test_save_env_value_sets_0600(self):
        ...
        with patch("zaya_cli.config.get_env_path", return_value=env_path), \
             patch("zaya_cli.config.ensure_zaya_home"):
    ...
    def test_ensure_zaya_home_sets_0700(self):
        home = Path(self.tmpdir) / ".zaya"
        with patch("zaya_cli.config.get_zaya_home", return_value=home):
