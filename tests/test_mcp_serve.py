"""
Tests for mcp_serve — Zaya MCP server.
...
def _isolate_zaya_home(tmp_path, monkeypatch):
    """Redirect ZAYA_HOME to a temp directory."""
    monkeypatch.setenv("ZAYA_HOME", str(tmp_path))
    try:
        import zaya_constants
        monkeypatch.setattr(zaya_constants, "get_zaya_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking zaya_state schema."""
