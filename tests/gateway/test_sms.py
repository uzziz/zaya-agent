...
class TestSmsToolset:
    def test_zaya_sms_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("zaya-sms")
        assert ts is not None
        assert "tools" in ts

    def test_zaya_sms_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("zaya-gateway")
        assert gw is not None
        assert "zaya-sms" in gw["includes"]
