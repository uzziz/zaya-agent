"""
Tests for timezone support (zaya_time module + integration points).
...
def _reset_zaya_time_cache():
    """Reset the zaya_time module cache (replacement for removed reset_cache)."""
    zaya_time._cached_tz = None
    zaya_time._cached_tz_name = None
    zaya_time._cache_resolved = False


# =========================================================================
# zaya_time.now() — core helper
# =========================================================================

class TestZayaTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_zaya_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["ZAYA_TIMEZONE"] = "Asia/Kolkata"
        result = zaya_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["ZAYA_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="zaya_time"):
            result = zaya_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["ZAYA_TIMEZONE"] = "Europe/London"
        tz = zaya_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["ZAYA_TIMEZONE"] = "Not/A/Timezone"
        tz = zaya_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When ZAYA_TIMEZONE is set, child process sees TZ env var."""
        import json as _json
        os.environ["ZAYA_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_tz_not_injected_when_empty(self):
        """When ZAYA_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("ZAYA_TIMEZONE", None)
    ...
    def test_zaya_timezone_not_leaked_to_child(self):
        """ZAYA_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["ZAYA_TIMEZONE"] = "Asia/Kolkata"

        with patch("model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("ZAYA_TIMEZONE", "NOT_SET"))',
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=zaya_tz) which shifted
        absolute time when system-local tz != Zaya tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_zaya_tz(self):
        """Already-aware datetimes should be normalized to Zaya tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Zaya tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Zaya tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Zaya is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Zaya tz
        ...
        # Use a Zaya timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _zaya_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["ZAYA_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_zaya_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Zaya timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["ZAYA_TIMEZONE"] = "US/Eastern"
        _reset_zaya_time_cache()
