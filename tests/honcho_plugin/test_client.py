...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "zaya"
        assert config.workspace_id == "zaya"
...
class TestFromGlobalConfig:
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"zaya": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="zaya-agent"
        ):
            result = config.resolve_session_name("/home/user/zaya-agent/subdir")
        assert result == "zaya-agent"
...
class TestResolveConfigPath:
    def test_prefers_zaya_home_when_exists(self, tmp_path):
        zaya_home = tmp_path / "zaya"
        zaya_home.mkdir()
        local_cfg = zaya_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        zaya_home = tmp_path / "zaya"
        zaya_home.mkdir()
        # No honcho.json in ZAYA_HOME — also isolate ~/.zaya so
    ...
    def test_falls_back_to_global_without_zaya_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("ZAYA_HOME", None)
            result = resolve_config_path()
        assert result == GLOBAL_CONFIG_PATH
...
class TestResolveActiveHost:
    def test_default_returns_zaya(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("ZAYA_HONCHO_HOST", None)
            os.environ.pop("ZAYA_HOME", None)
            assert resolve_active_host() == "zaya"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ZAYA_HONCHO_HOST": "zaya.coder"}):
            assert resolve_active_host() == "zaya.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZAYA_HONCHO_HOST", None)
            with patch("zaya_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "zaya.coder"

    def test_default_profile_returns_zaya(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZAYA_HONCHO_HOST", None)
            with patch("zaya_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "zaya"

    def test_custom_profile_returns_zaya(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZAYA_HONCHO_HOST", None)
            with patch("zaya_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "zaya"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZAYA_HONCHO_HOST", None)
            # Temporarily remove zaya_cli.profiles to simulate import failure
            saved = sys.modules.get("zaya_cli.profiles")
            sys.modules["zaya_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "zaya"
            finally:
                if saved is not None:
                    sys.modules["zaya_cli.profiles"] = saved
                else:
                    sys.modules.pop("zaya_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="zaya.coder")
        assert config.host == "zaya.coder"
        assert config.workspace_id == "zaya"  # shared workspace
        assert config.ai_peer == "zaya.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="zaya")
        assert config.host == "zaya"
        assert config.workspace_id == "zaya"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "zaya": {"aiPeer": "zaya", "peerName": "alice"},
                "zaya.coder": {
                    "aiPeer": "zaya.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="zaya.coder", config_path=config_file,
        )
        assert config.host == "zaya.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "zaya.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="zaya.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "zaya.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"zaya": {"enabled": True, "aiPeer": "zaya"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"zaya": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestInitOnSessionStart:
    ...
    def test_host_block_overrides_root(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "initOnSessionStart": True,
            "hosts": {"zaya": {"initOnSessionStart": False}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
    ...
    def test_host_block_true_overrides_root_absent(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"zaya": {"initOnSessionStart": True}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.init_on_session_start is True
