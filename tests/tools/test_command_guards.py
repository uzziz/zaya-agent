...
def _clean_state():
    ...
    saved = {}
    for k in ("ZAYA_INTERACTIVE", "ZAYA_GATEWAY_SESSION", "ZAYA_EXEC_ASK", "ZAYA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ZAYA_INTERACTIVE"] = "1"
        session_key = os.getenv("ZAYA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ZAYA_INTERACTIVE or ZAYA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
