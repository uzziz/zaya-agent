...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zaya_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_zaya_bin(self):
        """After _INSTALL_FAILED, manual install in ZAYA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        zaya_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(zaya_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(zaya_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zaya_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == zaya_bin
            assert _tirith_mod._resolved_path == zaya_bin
            mock_clear.assert_called_once()
...
class TestZayaHomeIsolation:
    def test_zaya_bin_dir_respects_zaya_home(self):
        """_zaya_bin_dir must use ZAYA_HOME, not hardcoded ~/.zaya."""
        from tools.tirith_security import _zaya_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ZAYA_HOME": tmpdir}):
            result = _zaya_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_zaya_home(self):
        """_failure_marker_path must use ZAYA_HOME, not hardcoded ~/.zaya."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ZAYA_HOME": "/custom/zaya"}):
            result = _failure_marker_path()
        assert result == "/custom/zaya/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ZAYA_HOME; verify it's active."""
        zaya_home = os.getenv("ZAYA_HOME")
        assert zaya_home is not None, "ZAYA_HOME should be set by conftest"
        assert "zaya_test" in zaya_home, "Should point to test temp dir"

    def test_get_zaya_home_fallback(self):
        """Without ZAYA_HOME set, falls back to ~/.zaya."""
        from tools.tirith_security import _get_zaya_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ZAYA_HOME entirely
            os.environ.pop("ZAYA_HOME", None)
            result = _get_zaya_home()
        assert result == os.path.join(os.path.expanduser("~"), ".zaya")
