...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        for key in (
            "ZAYA_ENABLE_NOUS_MANAGED_TOOLS",
            "FIRECRAWL_API_KEY",
            "FIRECRAWL_API_URL",
        ...
        os.environ["ZAYA_ENABLE_NOUS_MANAGED_TOOLS"] = "1"
    ...
    def test_nous_auth_token_respects_zaya_home_override(self, tmp_path):
        """Auth lookup should read from ZAYA_HOME/auth.json, not ~/.zaya/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".zaya").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``zaya tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def setup_method(self):
        ...
        for key in self._ENV_KEYS:
            if key != "ZAYA_ENABLE_NOUS_MANAGED_TOOLS":
                os.environ.pop(key, None)
