...
def _restore_tool_modules():
    original_zaya_home = os.environ.get("ZAYA_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_zaya_home is None:
            os.environ.pop("ZAYA_HOME", None)
        else:
            os.environ["ZAYA_HOME"] = original_zaya_home
        _reset_modules(("tools", "zaya_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    zaya_cli = types.ModuleType("zaya_cli")
    zaya_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["zaya_cli"] = zaya_cli
    zaya_home = tmp_path / "zaya-home"
    os.environ["ZAYA_HOME"] = str(zaya_home)
    sys.modules["zaya_cli.config"] = types.SimpleNamespace(
        get_zaya_home=lambda: zaya_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="zaya-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": zaya_home / "modal_snapshots.json",
