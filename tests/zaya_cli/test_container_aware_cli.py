...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ZAYA_HOME with .container-mode file."""
    zaya_home = tmp_path / ".zaya"
    zaya_home.mkdir()
    monkeypatch.setenv("ZAYA_HOME", str(zaya_home))
    monkeypatch.delenv("ZAYA_DEV", raising=False)

    container_mode = zaya_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=zaya-agent\n"
        "exec_user=zaya\n"
        "zaya_bin=/data/current-package/bin/zaya\n"
    )
    return zaya_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("zaya_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "zaya-agent"
    assert info["exec_user"] == "zaya"
    assert info["zaya_bin"] == "/data/current-package/bin/zaya"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("zaya_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_zaya_dev(container_env, monkeypatch):
    """Returns None when ZAYA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ZAYA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_zaya_dev_zero(container_env, monkeypatch):
    """ZAYA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ZAYA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        zaya_home = Path(tmpdir) / ".zaya"
        zaya_home.mkdir()
        (zaya_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("zaya_constants.is_container", return_value=False), \
             patch("zaya_cli.config.get_zaya_home", return_value=zaya_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZAYA_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=zaya-custom\n"
        "exec_user=myuser\n"
        "zaya_bin=/opt/zaya/bin/zaya\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "zaya-custom"
    assert info["exec_user"] == "myuser"
    assert info["zaya_bin"] == "/opt/zaya/bin/zaya"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from zaya_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "zaya"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "zaya-agent" in cmd
    assert "/data/current-package/bin/zaya" in cmd
    assert "chat" in cmd
