...
import zaya_cli.doctor as doctor
import zaya_cli.gateway as gateway_cli
from zaya_cli import doctor as doctor_mod
from zaya_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "ZAYA_HOME", zaya_home)
    monkeypatch.delenv("ZAYA_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("ZAYA_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_zaya_home(self, tmp_path, provider=""):
        """Create a minimal ZAYA_HOME with config.yaml."""
        home = tmp_path / ".zaya"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_zaya_home(tmp_path, provider)
