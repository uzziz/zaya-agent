"""Tests for ``zaya debug`` CLI command and debug utilities."""
...
def zaya_home(tmp_path, monkeypatch):
    """Set up an isolated ZAYA_HOME with minimal logs."""
    home = tmp_path / ".zaya"
    home.mkdir()
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from zaya_cli.debug import _upload_paste_rs
        ...
        with patch("zaya_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "zaya_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from zaya_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from zaya_cli.debug import upload_to_pastebin

        with patch("zaya_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("zaya_cli.debug._upload_paste_rs",
             ...
             patch("zaya_cli.debug._upload_dpaste_com",
...
class TestReadFullLog:
    ...
    def test_reads_small_file(self, zaya_home):
        from zaya_cli.debug import _read_full_log
    ...
    def test_returns_none_for_empty(self, zaya_home):
        # Truncate agent.log to empty
        (zaya_home / "logs" / "agent.log").write_text("")

        from zaya_cli.debug import _read_full_log
        assert _read_full_log("agent") is None

    def test_truncates_large_file(self, zaya_home):
        """Files larger than max_bytes get tail-truncated."""
        from zaya_cli.debug import _read_full_log

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (zaya_home / "logs" / "agent.log").write_text(big_content * 200)

        content = _read_full_log("agent", max_bytes=1024)
        assert content is not None
        assert "truncated" in content

    def test_unknown_log_returns_none(self, zaya_home):
        from zaya_cli.debug import _read_full_log
        assert _read_full_log("nonexistent") is None

    def test_falls_back_to_rotated_file(self, zaya_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from zaya_cli.debug import _read_full_log

        logs_dir = zaya_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, zaya_home):
    ...
    def test_falls_back_when_primary_empty(self, zaya_home):
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, zaya_home):
        from zaya_cli.debug import collect_debug_report

        with patch("zaya_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- zaya dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- zaya dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, zaya_home):
        from zaya_cli.debug import collect_debug_report

        with patch("zaya_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, zaya_home):
        from zaya_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, zaya_home):
        from zaya_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_local_flag_prints_full_logs(self, zaya_home, capsys):
    ...
    def test_share_uploads_three_pastes(self, zaya_home, capsys):
        ...
        with patch("zaya_cli.dump.run_dump") as mock_dump, \
             patch("zaya_cli.debug.upload_to_pastebin",
                    side_effect=_mock_upload):
            mock_dump.side_effect = lambda a: print("--- zaya dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, zaya_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, zaya_home, capsys):
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "zaya debug share" in out

    def test_share_subcommand_routes(self, zaya_home):
        from zaya_cli.debug import run_debug
