"""Tests for Codex auth — tokens stored in Zaya auth store (~/.zaya/auth.json)."""
...
def _setup_zaya_auth(zaya_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Zaya auth store."""
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    zaya_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (zaya_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("ZAYA_HOME", str(zaya_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    zaya_home = tmp_path / "zaya"
    _setup_zaya_auth(zaya_home, access_token="")
    monkeypatch.setenv("ZAYA_HOME", str(zaya_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_zaya_auth(zaya_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("ZAYA_HOME", str(zaya_home))
    ...
    monkeypatch.setattr("zaya_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    zaya_home = tmp_path / "zaya"
    _setup_zaya_auth(zaya_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("ZAYA_HOME", str(zaya_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Zaya auth store, not ~/.codex/."""
    zaya_home = tmp_path / "zaya"
    codex_home = tmp_path / "codex-cli"
    zaya_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "zaya-at", "refresh_token": "zaya-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Zaya store
    assert not (codex_home / "auth.json").exists()

    # Zaya auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "zaya-at"
...
def test_refresh_codex_auth_tokens_writes_back_to_cli(tmp_path, monkeypatch):
    ...
    from zaya_cli.auth import _refresh_codex_auth_tokens
    ...
    monkeypatch.setattr("zaya_cli.auth.refresh_codex_oauth_pure", lambda *a, **kw: {
...
def test_resolve_returns_zaya_auth_store_source(tmp_path, monkeypatch):
    zaya_home = tmp_path / "zaya"
    _setup_zaya_auth(zaya_home)
    monkeypatch.setenv("ZAYA_HOME", str(zaya_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "zaya-auth-store"
