...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("zaya/zaya-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned zaya/* and pr-* branches."""

    def test_prunes_orphaned_zaya_branch(self, git_repo):
        """zaya/zaya-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "zaya/zaya-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "zaya/zaya-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "zaya/zaya-deadbeef" in result.stdout
        ...
        assert "zaya/zaya-deadbeef" in orphaned
        ...
        assert "zaya/zaya-deadbeef" not in result.stdout
