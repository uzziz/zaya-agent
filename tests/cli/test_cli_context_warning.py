...
def _isolate(tmp_path, monkeypatch):
    """Isolate ZAYA_HOME so tests don't touch real config."""
    home = tmp_path / ".zaya"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal ZayaCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ZayaCLI.__new__(ZayaCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["zaya-core"]
        obj.compact = False
