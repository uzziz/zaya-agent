...
if tinker_atropos_dir.exists():
    ...
    os.environ['ZAYA_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to zaya-agent directory if submodule not found
...
from zaya_constants import get_zaya_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_zaya_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.zaya/config.yaml if not provided)
    ...
    """
    # Load config from ~/.zaya/config.yaml
    config = load_zaya_config()
