"""ACP agent server — exposes Zaya Agent via the Agent Client Protocol."""
...
try:
    from zaya_cli import __version__ as ZAYA_VERSION
except Exception:
    ZAYA_VERSION = "0.0.0"
...
class ZayaACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Zaya AIAgent."""
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = getattr(state.agent, "enabled_toolsets", None) or ["zaya-acp"]
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Zaya using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="zaya-agent", version=ZAYA_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Zaya on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from zaya_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            from model_tools import get_tool_definitions
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["zaya-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Zaya Agent v{ZAYA_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Zaya has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
