---
sidebar_position: 8
title: "Use Voice Mode with Zaya"
description: "A practical guide to setting up and using Zaya voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Zaya
...
- you want Zaya sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Zaya.
...
## Step 1: make sure normal Zaya works first
...
Zaya supports both local and cloud speech stacks.
...
### If you use `zaya setup`

If you choose NeuTTS in the setup wizard, Zaya checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Zaya transcribes and responds
...
- asking Zaya to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Zaya loop.

## Tuning CLI behavior

### Silence threshold

If Zaya starts/stops too aggressively, tune:
...
Zaya stays a normal chat bot, but can speak replies.
...
- you want Zaya to function like a portable research or ops assistant
...
Zaya joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Zaya detects speech boundaries
- transcripts are posted in the associated text channel
- Zaya responds in text and audio
...
1. get text Zaya working
2. install `zaya-agent[voice]`
