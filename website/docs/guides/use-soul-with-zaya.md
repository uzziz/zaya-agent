---
sidebar_position: 7
title: "Use SOUL.md with Zaya"
description: "How to use SOUL.md to shape Zaya Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Zaya

`SOUL.md` is the **primary identity** for your Zaya instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Zaya to feel like the same assistant every time you talk to it — or if you want to replace the Zaya persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Zaya should be
- what Zaya should avoid stylistically
- how Zaya should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Zaya is and how Zaya speaks
...
Zaya now uses only the global SOUL file for the current instance:

```text
~/.zaya/SOUL.md
```

If you run Zaya with a custom home directory, it becomes:
...
Zaya automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Zaya does not overwrite it
- if the file exists but is empty, Zaya adds nothing from it to the prompt

## How Zaya uses it

When Zaya starts a session, it reads `SOUL.md` from `ZAYA_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Zaya feels.
...
Zaya already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Zaya should sound.

# Avoid
What Zaya should not do.

# Defaults
How Zaya should behave when ambiguity appears.
```
...
Then restart Zaya or start a new session.
...
### I edited SOUL.md but Zaya still sounds the same

Check:
- you edited `~/.zaya/SOUL.md` or `$ZAYA_HOME/SOUL.md`
...
### Zaya is ignoring parts of my SOUL.md
