...
This tutorial walks you through setting up a Telegram bot powered by Zaya Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Zaya Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.zaya/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Zaya itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Zaya Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_zaya_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Zaya Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u zaya-gateway -f
...
zaya gateway start
zaya gateway stop
tail -f ~/.zaya/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `zaya gateway install` to update the plist.
:::
...
```bash
zaya gateway stop && zaya gateway start
```
...
zaya pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.zaya/.env`:
...
Control how much detail the bot shows when using tools. In `~/.zaya/config.yaml`:
...
Customize how the bot communicates by editing `~/.zaya/SOUL.md`:

For a full guide, see [Use SOUL.md with Zaya](/docs/guides/use-soul-with-zaya).
...
```bash
# From the CLI
zaya cron list          # View all scheduled jobs
zaya cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u zaya-gateway` (Linux) or `~/.zaya/logs/gateway.log` (macOS) |
| Cron job output | `~/.zaya/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.zaya/cron/jobs.json` |
| Pairing data | `~/.zaya/pairing/` |
| Session history | `~/.zaya/sessions/` |
