...
title: "Build a Zaya Plugin"
description: "Step-by-step guide to building a complete Zaya plugin with tools, hooks, data files, and skills"
---

# Build a Zaya Plugin

This guide walks through building a complete Zaya plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
```bash
mkdir -p ~/.zaya/plugins/calculator
cd ~/.zaya/plugins/calculator
```
...
This tells Zaya: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Zaya may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `zaya my-plugin <subcommand>`)
- If this function crashes, the plugin is disabled but Zaya continues fine
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.zaya/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.zaya/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `zaya plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Zaya injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: zaya-agent"}
...
- **The system prompt is Zaya's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `zaya <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for zaya my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: zaya my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for zaya my-plugin."""
...
After registration, users can run `zaya my-plugin status`, `zaya my-plugin config`, etc.
...
```toml
# pyproject.toml
[project.entry-points."zaya_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install zaya-plugin-calculator
# Plugin auto-discovered on next zaya startup
```
...
```python
# Wrong — will break if Zaya passes extra context
def handler(args):
    ...
