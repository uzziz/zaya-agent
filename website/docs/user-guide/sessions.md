...
Zaya Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.zaya/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.zaya/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`zaya` or `zaya chat`) |
...
zaya -c "my project"   # → resumes "my project #3"
```
...
zaya --resume 20250305_091523_a1b2c3d4
zaya -r 20250305_091523_a1b2c3d4

# Resume by title
zaya --resume "refactoring auth"

# Or with the chat subcommand
zaya chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `zaya sessions list`.

### Conversation Recap on Resume

When you resume a session, Zaya displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Zaya session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.zaya/config.yaml`:
...
Zaya automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `zaya sessions list` or `zaya sessions browse`.
...
```bash
zaya sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Zaya creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`zaya -c "my project"`), it automatically picks the most recent session in the lineage.
...
Zaya provides a full set of session management commands via `zaya sessions`:
...
# Filter by platform
zaya sessions list --source telegram

# Show more sessions
zaya sessions list --limit 50
```
...
zaya sessions export backup.jsonl

# Export sessions from a specific platform
zaya sessions export telegram-history.jsonl --source telegram

# Export a single session
zaya sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
zaya sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
zaya sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
zaya sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
zaya sessions prune --older-than 30

# Only prune sessions from a specific platform
zaya sessions prune --source telegram --older-than 60

# Skip confirmation
zaya sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`zaya insights`](/docs/reference/cli-commands#zaya-insights).
...
When Zaya cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Zaya uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Zaya in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.zaya/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.zaya/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.zaya/sessions/sessions.json` | Maps session keys to active session IDs |
...
zaya sessions delete <session_id>
