...
title: "Git Worktrees"
description: "Run multiple Zaya agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Zaya Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Zaya so each session has a clean, isolated working directory.

## Why Use Worktrees with Zaya?

Zaya treats the **current working directory** as the project root:

- CLI: the directory where you run `zaya` or `zaya chat`
...
git worktree add ../repo-feature feature/zaya-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/zaya-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Zaya there:

```bash
cd ../repo-feature

# Start Zaya in the worktree
zaya
```
...
git worktree add ../repo-experiment-a feature/zaya-a
git worktree add ../repo-experiment-b feature/zaya-b
```
...
- Works on its own branch (`feature/zaya-a` vs `feature/zaya-b`).
...
- Zaya checkpoint data under `~/.zaya/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Zaya experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/zaya-checkpoints-docs`, `feature/zaya-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Zaya from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `zaya -w` (Automatic Worktree Mode)

Zaya has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `zaya/zaya-<hash>`).
...
```bash
zaya -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `zaya -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Zaya session its own clean checkout.
