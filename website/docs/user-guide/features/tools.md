...
description: "Overview of Zaya Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
Zaya ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
zaya chat --toolsets "web,terminal"
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `zaya-cli`, `zaya-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
# Configure
zaya config set terminal.backend singularity
zaya config set terminal.singularity_image ~/python.sif
```
...
modal setup
zaya config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.zaya/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.zaya/.env`.
:::
