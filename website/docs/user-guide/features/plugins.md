...
title: "Plugins"
description: "Extend Zaya with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Zaya has a plugin system for adding custom tools, hooks, and integrations without modifying core code.

**→ [Build a Zaya Plugin](/docs/guides/build-a-zaya-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.zaya/plugins/` with a `plugin.yaml` and Python code:
...
Start Zaya — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.zaya/plugins/hello-world/plugin.yaml`**
...
**`~/.zaya/plugins/hello-world/__init__.py`**

```python
"""Minimal Zaya plugin — registers a tool and a hook."""
...
Drop both files into `~/.zaya/plugins/hello-world/`, restart Zaya, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.zaya/plugins/` are disabled by default. Enable them only for trusted repositories by setting `ZAYA_ENABLE_PROJECT_PLUGINS=true` before starting Zaya.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `zaya <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `zaya plugins install` |
| Distribute via pip | `[project.entry-points."zaya_agent.plugins"]` |

## Plugin discovery

| Source | Path | Use case |
|--------|------|----------|
| User | `~/.zaya/plugins/` | Personal plugins |
| Project | `.zaya/plugins/` | Project-specific plugins (requires `ZAYA_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `zaya_agent.plugins` entry_points | Distributed packages |
...
Zaya has three kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, CLI commands | Multi-select (enable/disable) | `~/.zaya/plugins/` |
...
```bash
zaya plugins                  # unified interactive UI
zaya plugins list             # table view with enabled/disabled status
zaya plugins install user/repo  # install from Git
zaya plugins update my-plugin   # pull latest
zaya plugins remove my-plugin   # uninstall
zaya plugins enable my-plugin   # re-enable a disabled plugin
zaya plugins disable my-plugin  # disable without removing
```

### Interactive UI

Running `zaya plugins` with no arguments opens a composite interactive screen:
...
See the **[full guide](/docs/guides/build-a-zaya-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
