...
Schedule tasks to run automatically with natural language or cron expressions. Zaya exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Zaya disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
zaya cron create "every 2h" "Check server status"
zaya cron create "every 1h" "Summarize new feed items" --skill blogwatcher
zaya cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill find-nearby \
  --name "Skill combo"
```
...
Zaya will use the unified `cronjob` tool internally.
...
```bash
zaya cron edit <job_id> --schedule "every 4h"
zaya cron edit <job_id> --prompt "Use the revised task"
zaya cron edit <job_id> --skill blogwatcher --skill find-nearby
zaya cron edit <job_id> --add-skill find-nearby
zaya cron edit <job_id> --remove-skill blogwatcher
zaya cron edit <job_id> --clear-skills
```
...
```bash
zaya cron list
zaya cron pause <job_id>
zaya cron resume <job_id>
zaya cron run <job_id>
zaya cron remove <job_id>
zaya cron status
zaya cron tick
```
...
sudo zaya gateway install --system   # Linux: boot-time system service for servers
zaya gateway             # Or run in foreground
...
1. loads jobs from `~/.zaya/cron/jobs.json`
...
A file lock at `~/.zaya/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.zaya/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.zaya/cron/output/`), but no message is sent to the delivery target.
...
Or set the `ZAYA_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Zaya skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.zaya/cron/jobs.json`. Output from job runs is saved to `~/.zaya/cron/output/{job_id}/{timestamp}.md`.
