...
All skills live in **`~/.zaya/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Zaya at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example of a skill-backed slash command with custom behavior. Running `/plan [request]` tells Zaya to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.zaya/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
zaya chat --toolsets skills -q "What skills do you have?"
zaya chat --toolsets skills -q "Show me the axolotl skill"
```
...
When a missing value is encountered, Zaya asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `zaya setup` or `~/.zaya/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `zaya config migrate` prompts for unconfigured settings, and `zaya config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.zaya/skills/                  # Single source of truth
...
If you maintain skills outside of Zaya — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Zaya to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.zaya/config.yaml`:
...
- **Read-only**: External dirs are only scanned for skill discovery. When the agent creates or edits a skill, it always writes to `~/.zaya/skills/`.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Zaya ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.zaya/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
```bash
zaya skills browse                              # Browse all hub skills (official first)
zaya skills browse --source official            # Browse only official optional skills
zaya skills search kubernetes                   # Search all sources
zaya skills search react --source skills-sh     # Search the skills.sh directory
zaya skills search https://mintlify.com/docs --source well-known
zaya skills inspect openai/skills/k8s           # Preview before installing
zaya skills install openai/skills/k8s           # Install with security scan
zaya skills install official/security/1password
zaya skills install skills-sh/vercel-labs/json-render/json-render-react --force
zaya skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
zaya skills list --source hub                   # List hub-installed skills
zaya skills check                               # Check installed hub skills for upstream updates
zaya skills update                              # Reinstall hub skills with upstream changes when needed
zaya skills audit                               # Re-scan all hub skills for security
zaya skills uninstall k8s                       # Remove a hub skill
zaya skills publish skills/my-skill --to github --repo owner/repo
zaya skills snapshot export setup.json          # Export skill config
zaya skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Zaya. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `zaya skills search <query> --source skills-sh`. Zaya resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Zaya currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Zaya repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Zaya can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
zaya skills search https://mintlify.com/docs --source well-known
zaya skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
zaya skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Zaya can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
zaya skills install openai/skills/k8s
zaya skills tap add myorg/skills-repo
```
...
- Zaya source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Zaya supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Zaya source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Zaya can search and convert agent entries from LobeHub's public catalog into installable Zaya skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Zaya source id: `lobehub`
...
`zaya skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Zaya | Always trusted |
...
```bash
zaya skills check          # Report which installed hub skills changed upstream
zaya skills update         # Reinstall only the skills with updates available
zaya skills update react   # Update one specific installed hub skill
```
