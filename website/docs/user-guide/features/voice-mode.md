...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Zaya Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Zaya Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Zaya](/docs/guides/use-voice-mode-with-zaya).

## Prerequisites

Before using voice features, make sure you have:

1. **Zaya Agent installed** — `pip install zaya-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `zaya model` or set your preferred provider credentials in `~/.zaya/.env`
3. **A working base setup** — run `zaya` to verify the agent responds to text before enabling voice

:::tip
The `~/.zaya/` directory and default `config.yaml` are created automatically the first time you run `zaya`. You only need to create `~/.zaya/.env` manually for API keys.
:::
...
pip install "zaya-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "zaya-agent[all]"
```
...
```bash
zaya                # Start the interactive CLI
```
...
1. Start the CLI with `zaya` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.zaya/config.yaml` (default: `ctrl+b`).
:::
...
```bash
zaya gateway        # Start the gateway (connects to configured platforms)
zaya gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@zayabyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.zaya/.env`:
```bash
...
```bash
zaya gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.zaya/.env`
...
- Review gateway logs: `tail -f ~/.zaya/logs/gateway.log`
