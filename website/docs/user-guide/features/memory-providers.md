...
Zaya Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
zaya memory setup      # interactive picker + configuration
zaya memory status     # check what's active
zaya memory off        # disable external provider
```

You can also select the active memory provider via `zaya plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.zaya/config.yaml`:
...
When a memory provider is active, Zaya automatically:
...
**Setup Wizard:**
```bash
zaya honcho setup        # (legacy command) 
# or
zaya memory setup        # select "honcho"
```

**Config:** `$ZAYA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$ZAYA_HOME/honcho.json` > `~/.zaya/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/zaya-ai/zaya-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/zaya).
...
:::tip Migrating from `zaya honcho`
If you previously used `zaya honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-agent / Profiles:**

Each Zaya profile gets its own Honcho AI peer while sharing the same workspace -- all profiles see the same user representation, but each agent builds its own identity and observations.

```bash
zaya profile create coder --clone   # creates honcho peer "coder", inherits config from default
```

What `--clone` does: creates a `zaya.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The peer is eagerly created in Honcho so it exists before first message.

For profiles created before Honcho was set up:

```bash
zaya honcho sync   # scans all profiles, creates host blocks for any missing ones
```

This inherits settings from the default `zaya` host block and creates new AI peers for each profile. Idempotent -- skips profiles that already have a host block.
...
See the [config reference](https://github.com/zaya-ai/zaya-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/zaya).
...
# Then configure Zaya
zaya memory setup    # select "openviking"
# Or manually:
zaya config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.zaya/.env
```
...
**Config:** `$ZAYA_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `zaya-user` | User identifier |
...
**Setup:**
```bash
zaya memory setup    # select "hindsight"
# Or manually:
zaya config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.zaya/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p zaya ui start`

**Config:** `$ZAYA_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `zaya` | Memory bank identifier |
...
See [plugin README](https://github.com/NousResearch/zaya-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Config:** `config.yaml` under `plugins.zaya-memory-store`
...
**Setup:**
```bash
zaya memory setup    # select "retaindb"
# Or manually:
zaya config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.zaya/.env
```
...
# Then configure Zaya
zaya memory setup    # select "byterover"
# Or manually:
zaya config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ZAYA_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
zaya memory setup    # select "supermemory"
# Or manually:
zaya config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.zaya/.env
```

**Config:** `$ZAYA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `zaya` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `zaya-{identity}` → `zaya-coder`) to isolate memories per Zaya profile
...
- **Local storage providers** (Holographic, ByteRover) use `$ZAYA_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ZAYA_HOME/` so each profile has its own credentials
