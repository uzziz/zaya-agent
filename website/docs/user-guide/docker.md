...
sidebar_position: 7
title: "Docker"
description: "Running Zaya Agent in Docker and using Docker as a terminal backend"
---

# Zaya Agent — Docker

There are two distinct ways Docker intersects with Zaya Agent:

1. **Running Zaya IN Docker** — the agent itself runs inside a container (this page's primary focus)
...
If this is your first time running Zaya Agent, create a data directory on the host and start the container interactively to run the setup wizard:

```sh
mkdir -p ~/.zaya
docker run -it --rm \
  -v ~/.zaya:/opt/data \
  nousresearch/zaya-agent setup
```

This drops you into the setup wizard, which will prompt you for your API keys and write them to `~/.zaya/.env`. You only need to do this once. It is highly recommended to set up a chat system for the gateway to work with at this point.
...
docker run -d \
  ...
  -v ~/.zaya:/opt/data \
  -p 8642:8642 \
  nousresearch/zaya-agent gateway run
```
...
docker run -d \
  ...
  nousresearch/zaya-agent dashboard
```
...
docker run -it --rm \
  -v ~/.zaya:/opt/data \
  nousresearch/zaya-agent
```

## Persistent volumes

The `/opt/data` volume is the single source of truth for all Zaya state. It maps to your host's `~/.zaya/` directory and contains:

| Path | Contents |
|------|----------|
| `.env` | API keys and secrets |
| `config.yaml` | All Zaya configuration |
...
:::warning
Never run two Zaya **gateway** containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent write access. Running a dashboard container alongside the gateway is safe since the dashboard only reads data.
:::
...
services:
  zaya:
    image: nousresearch/zaya-agent:latest
    container_name: zaya
    restart: unless-stopped
    command: gateway run
    ports:
  ...
  dashboard:
    ...
    volumes:
      - ~/.zaya:/opt/data
    environment:
      - GATEWAY_HEALTH_URL=http://zaya:8642
    networks:
      - zaya-net
    depends_on:
      - zaya
...
The Zaya container needs moderate resources. Recommended minimums:
...
- Python 3 with all Zaya dependencies (`pip install -e ".[all]"`)
...
- Then runs `zaya` with whatever arguments you pass
...
```sh
docker pull nousresearch/zaya-agent:latest
docker rm -f zaya
docker run -d \
  --name zaya \
  --restart unless-stopped \
...
Check logs: `docker logs zaya`. Common causes:
- Missing or invalid `.env` file — run interactively first to complete setup
- Port conflicts if running with exposed ports

### "Permission denied" errors

The container runs as root by default. If your host `~/.zaya/` was created by a non-root user, permissions should work. If you get errors, ensure the data directory is writable:
...
```sh
docker logs --tail 50 zaya          # Recent logs
docker run -it --rm nousresearch/zaya-agent:latest version     # Verify version
docker stats zaya                    # Resource usage
```
