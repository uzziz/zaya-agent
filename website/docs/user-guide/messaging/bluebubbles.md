...
Connect Zaya to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Zaya and the BlueBubbles server
...
Or set environment variables directly in `~/.zaya/.env`:
...
When someone messages your iMessage, Zaya automatically sends them a pairing code. Approve it with:
```bash
zaya pairing approve bluebubbles <CODE>
```
Use `zaya pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.zaya/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.zaya/.env`):
```bash
...
Zaya will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Zaya
Zaya → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Zaya sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Zaya resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `zaya logs gateway` for webhook errors (or `zaya logs -f` to follow in real-time)
