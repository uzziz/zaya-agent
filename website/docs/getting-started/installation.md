...
sidebar_position: 2
title: "Installation"
description: "Install Zaya Agent on Linux, macOS, WSL2, or Android via Termux"
---

# Installation

Get Zaya Agent up and running in under two minutes with the one-line installer, or follow the manual steps for full control.
...
Zaya now ships a Termux-aware installer path too:
...
:::warning Windows
Native Windows is **not supported**. Please install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Zaya Agent from there. The install command above works inside WSL2.
:::

### What the Installer Does

The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `zaya` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
zaya gateway setup  # Set up messaging platforms
zaya config set     # Set individual config values
zaya setup          # Or run the full setup wizard to configure everything at once
```
...
:::tip
You do **not** need to activate the venv to use `zaya`. The entry point has a hardcoded shebang pointing to the venv Python, so it works globally once symlinked.
:::
...
mkdir -p ~/.zaya/{cron,sessions,logs,memories,skills,pairing,hooks,image_cache,audio_cache,whatsapp/session}
...
Open `~/.zaya/.env` and add at minimum an LLM provider key:
...
```bash
zaya config set OPENROUTER_API_KEY sk-or-v1-your-key-here
```

### Step 8: Add `zaya` to Your PATH
...
```bash
zaya model       # Select your LLM provider and model
```

### Step 10: Verify the Installation

```bash
zaya version    # Check that the command is available
zaya doctor     # Run diagnostics to verify everything is working
zaya status     # Check your configuration
zaya chat -q "Hello! What tools do you have available?"
```
...
| Problem | Solution |
|---------|----------|
| `zaya: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `zaya model` to configure your provider, or `zaya config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `zaya config check` then `zaya config migrate` |

For more diagnostics, run `zaya doctor` — it will tell you exactly what's missing and how to fix it.
