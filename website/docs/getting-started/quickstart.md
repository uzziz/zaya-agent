...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Zaya Agent — from install to chatting in 2 minutes"
---

# Quickstart

This guide walks you through installing Zaya Agent, setting up a provider, and having your first conversation. By the end, you'll know the key features and how to explore further.

## 1. Install Zaya Agent
...
zaya setup       # Or configure everything at once
```

`zaya model` walks you through selecting an inference provider:

| Provider | What it is | How to set up |
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `zaya model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `zaya model` |
| **Anthropic** | Claude models directly (Pro/Max or API key) | `zaya model` with Claude Code auth, or an Anthropic API key |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `zaya model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `zaya model` (requires `copilot` CLI + `copilot login`) |
| **Vercel AI Gateway** | Vercel AI Gateway routing | Set `AI_GATEWAY_API_KEY` |
| **Custom Endpoint** | VLLM, SGLang, Ollama, or any OpenAI-compatible API | Set base URL + API key |

:::caution Minimum context: 64K tokens
Zaya Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `zaya model` — no code changes, no lock-in. When configuring a custom endpoint, Zaya will prompt for the context window size and auto-detect it when possible. See [Context Length Detection](../integrations/providers.md#context-length-detection) for details.
:::
...
When you exit, zaya prints a resume command:

```bash
zaya --continue    # Resume the most recent session
zaya -c            # Short form
```
...
```bash
zaya config set terminal.backend docker    # Docker isolation
zaya config set terminal.backend ssh       # Remote server
```

### Connect messaging platforms

Chat with Zaya from your phone or other surfaces via Telegram, Discord, Slack, WhatsApp, Signal, Email, or Home Assistant:

```bash
zaya gateway setup    # Interactive platform configuration
```
...
```bash
pip install "zaya-agent[voice]"
# Includes faster-whisper for free local speech-to-text
```

Then start Zaya and enable it inside the CLI:

```text
/voice on
```

Press `Ctrl+B` to record, or use `/voice tts` to have Zaya speak its replies. See [Voice Mode](../user-guide/features/voice-mode.md) for the full setup across CLI, Telegram, Discord, and Discord voice channels.
...
### Use Zaya inside an editor via ACP

Zaya can also run as an ACP server for ACP-compatible editors like VS Code, Zed, and JetBrains:
...
| `zaya` | Start chatting |
| `zaya model` | Choose your LLM provider and model |
| `zaya tools` | Configure which tools are enabled per platform |
| `zaya setup` | Full setup wizard (configures everything at once) |
| `zaya doctor` | Diagnose issues |
| `zaya update` | Update to latest version |
| `zaya gateway` | Start the messaging gateway |
| `zaya --continue` | Resume last session |
