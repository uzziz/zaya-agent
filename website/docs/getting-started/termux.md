...
description: "Run Zaya Agent directly on an Android phone with Termux"
---

# Zaya on Android with Termux

This is the tested path for running Zaya Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Zaya from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Zaya now ships a Termux-aware installer path:
...
- links `zaya` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `zaya` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/zaya" "$PREFIX/bin/zaya"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `zaya` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.zaya/.env`.
...
### `zaya doctor` says ripgrep or Node is missing
