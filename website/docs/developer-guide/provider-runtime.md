...
description: "How Zaya resolves providers, credentials, API modes, and auxiliary models at runtime"
---

# Provider Runtime Resolution

Zaya has a shared provider runtime resolver used across:
...
- `zaya_cli/runtime_provider.py` — credential resolution, `_resolve_custom_runtime()`
- `zaya_cli/auth.py` — provider registry, `resolve_provider()`
- `zaya_cli/model_switch.py` — shared `/model` switch pipeline (CLI + gateway)
...
That ordering matters because Zaya treats the saved model/provider choice as the source of truth for normal runs. This prevents a stale shell export from silently overriding the endpoint a user last selected in `zaya model`.
...
This resolver is the main reason Zaya can share auth/runtime logic between:
...
Set `AI_GATEWAY_API_KEY` in `~/.zaya/.env` and run with `--provider ai-gateway`. Zaya fetches available models from the gateway's `/models` endpoint, filtering to language models with tool-use support.

## OpenRouter, AI Gateway, and custom OpenAI-compatible base URLs

Zaya contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY`, `AI_GATEWAY_API_KEY`, and `OPENAI_API_KEY`).
...
Zaya also distinguishes between:
...
When provider resolution selects `anthropic`, Zaya uses:
...
- Zaya preflights Anthropic credential refresh before native Messages API calls
- Zaya still retries once on a 401 after rebuilding the Anthropic client, as a fallback path
...
When an auxiliary task is configured with provider `main`, Zaya resolves that through the same shared runtime path as normal chat. In practice that means:

- env-driven custom endpoints still work
- custom endpoints saved via `zaya model` / `config.yaml` also work
- auxiliary routing can tell the difference between a real saved custom endpoint and the OpenRouter fallback

## Fallback models

Zaya supports a configured fallback model/provider pair, allowing runtime failover when the primary model encounters errors.
