...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Zaya-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Zaya Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Zaya/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- ZayaAgentBaseEnv
    ZayaAgentBaseEnv <|-- TerminalTestEnv
    ZayaAgentBaseEnv <|-- ZayaSweEnv
    ZayaAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The zaya-agent layer (`environments/zaya_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls zaya-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `ZayaAgentLoop` and scores the result
...
Your environment inherits from `ZayaAgentBaseEnv` and implements five methods:
...
`ZayaAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as zaya-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any zaya-agent tool |
...
parser = get_parser("zaya")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `zaya`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "zaya-agent[yc-bench]"
...
```bash
python environments/zaya_swe_env/zaya_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"zaya"`, `"qwen"`, `"mistral"`)
...
class MyEnvConfig(ZayaAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(ZayaAgentBaseEnv):
    name = "my-env"
...
### ZayaAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which zaya toolsets to enable |
...
| `tool_call_parser` | `str` | `"zaya"` | Parser for Phase 2 raw output |
...
env:
  ...
  tokenizer_name: "NousResearch/Zaya-3-Llama-3.1-8B"
  use_wandb: true
...
- [Modal](https://modal.com) account and CLI: `pip install "zaya-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "zaya-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── zaya_base_env.py          # Abstract base class (ZayaAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (ZayaAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── zaya_parser.py        # Zaya/ChatML <tool_call> format
...
├── zaya_swe_env/             # SWE-bench training environment
│
