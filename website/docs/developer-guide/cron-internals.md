...
description: "How Zaya stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `zaya_cli/cron.py` | CLI `zaya cron` subcommands |
...
Jobs are stored in `~/.zaya/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `zaya cron` commands are run or during active CLI sessions.
...
```python
# ~/.zaya/scripts/check_competitors.py
...
2. **Environment variable** — `ZAYA_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.zaya/cron/output/` |
...
The `zaya cron` CLI provides direct job management:

```bash
zaya cron list                    # Show all jobs
zaya cron create                  # Interactive job creation (alias: add)
zaya cron edit <job_id>           # Edit job configuration
zaya cron pause <job_id>          # Pause a running job
zaya cron resume <job_id>         # Resume a paused job
zaya cron run <job_id>            # Trigger immediate execution
zaya cron remove <job_id>         # Delete a job
```
