...
description: "How to add a new inference provider to Zaya Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Zaya can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `zaya model` menu entries
...
1. `zaya_cli/auth.py` decides how credentials are found.
2. `zaya_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `zaya_cli/models.py` and `zaya_cli/main.py` make the provider show up in the CLI. (`zaya_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `zaya_cli/auth.py`
2. `zaya_cli/models.py`
3. `zaya_cli/runtime_provider.py`
4. `zaya_cli/main.py`
...
:::tip
`zaya_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `zaya setup`.
:::
...
- `PROVIDER_REGISTRY` in `zaya_cli/auth.py`
- `_PROVIDER_LABELS` in `zaya_cli/models.py`
- `_PROVIDER_ALIASES` in both `zaya_cli/auth.py` and `zaya_cli/models.py`
- CLI `--provider` choices in `zaya_cli/main.py`
...
## Step 2: Add auth metadata in `zaya_cli/auth.py`
...
- What env vars should Zaya check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `zaya_cli/models.py`
...
## Step 4: Resolve runtime data in `zaya_cli/runtime_provider.py`
...
Be careful with API-key precedence. Zaya already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `zaya_cli/main.py`

A provider is not discoverable until it shows up in the interactive `zaya model` flow.

Update these in `zaya_cli/main.py`:
...
:::tip
`zaya_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `zaya model` and `zaya setup` automatically.
:::
...
When you add a native provider, double-check that Zaya is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m zaya_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m zaya_cli.main model
python -m zaya_cli.main setup
```
...
- [ ] `ProviderConfig` added in `zaya_cli/auth.py`
- [ ] aliases added in `zaya_cli/auth.py` and `zaya_cli/models.py`
- [ ] model catalog added in `zaya_cli/models.py`
- [ ] runtime branch added in `zaya_cli/runtime_provider.py`
- [ ] CLI wiring added in `zaya_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `zaya model` but not `zaya setup`
