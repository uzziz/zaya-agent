...
The ACP adapter wraps Zaya' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
zaya acp / zaya-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.zaya/.env
  -> configure stderr logging
  -> construct ZayaACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> Zaya `once`
- `allow_always` -> Zaya `always`
- reject options -> Zaya `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Zaya tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["zaya-acp"])
...
Instead it reuses Zaya' runtime resolver:

- `acp_adapter/auth.py`
- `zaya_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Zaya provider/credentials.
...
- `toolsets.py` — `zaya-acp` toolset definition
- `zaya_cli/main.py` — `zaya acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `zaya-acp` script
