...
description: "Reference for Zaya core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `zaya-cli` is the default for interactive CLI sessions)
...
```bash
zaya chat --toolsets web,file,terminal
zaya chat --toolsets debugging        # composite — expands to file + terminal + web
zaya chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - zaya-cli          # default for CLI
  # - zaya-telegram   # override for Telegram gateway
```

### Interactive management

```bash
zaya tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Zaya tools programmatically. |
...
Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `zaya-cli`:

| Toolset | Differences from `zaya-cli` |
|---------|-------------------------------|
| `zaya-cli` | Full toolset — all 36 tools including `clarify`. The default for interactive CLI sessions. |
| `zaya-acp` | Drops `clarify`, `cronjob`, `image_generate`, `send_message`, `text_to_speech`, homeassistant tools. Focused on coding tasks in IDE context. |
| `zaya-api-server` | Drops `clarify`, `send_message`, and `text_to_speech`. Adds everything else — suitable for programmatic access where user interaction isn't possible. |
| `zaya-telegram` | Same as `zaya-cli`. |
| `zaya-discord` | Same as `zaya-cli`. |
| `zaya-slack` | Same as `zaya-cli`. |
| `zaya-whatsapp` | Same as `zaya-cli`. |
| `zaya-signal` | Same as `zaya-cli`. |
| `zaya-matrix` | Same as `zaya-cli`. |
| `zaya-mattermost` | Same as `zaya-cli`. |
| `zaya-email` | Same as `zaya-cli`. |
| `zaya-sms` | Same as `zaya-cli`. |
| `zaya-dingtalk` | Same as `zaya-cli`. |
| `zaya-feishu` | Same as `zaya-cli`. |
| `zaya-wecom` | Same as `zaya-cli`. |
| `zaya-wecom-callback` | WeCom callback toolset — enterprise self-built app messaging (full access). |
| `zaya-weixin` | Same as `zaya-cli`. |
| `zaya-bluebubbles` | Same as `zaya-cli`. |
| `zaya-qqbot` | Same as `zaya-cli`. |
| `zaya-homeassistant` | Same as `zaya-cli`. |
| `zaya-webhook` | Same as `zaya-cli`. |
| `zaya-gateway` | Union of all messaging platform toolsets. Used internally when the gateway needs the broadest possible tool set. |
...
## Relationship to `zaya tools`

The `zaya tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
