...
description: "Official optional skills shipped with zaya-agent — install via zaya skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Official optional skills ship with the zaya-agent repository under `optional-skills/` but are **not active by default**. Install them explicitly:
...
```bash
zaya skills install official/blockchain/solana
zaya skills install official/mlops/flash-attention
```
...
```bash
zaya skills uninstall <skill-name>
```
...
| **honcho** | Configure and use Honcho memory with Zaya — cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. |
...
| Skill | Description |
|-------|-------------|
| **blender-mcp** | Control Blender directly from Zaya via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. |
...
| Skill | Description |
|-------|-------------|
| **openclaw-migration** | Migrate a user's OpenClaw customization footprint into Zaya Agent. Imports memories, SOUL.md, command allowlists, user skills, and selected workspace assets. |
...
| **zaya-atropos-environments** | Build, test, and debug Zaya Agent RL environments for Atropos training. Covers the ZayaAgentBaseEnv interface, reward functions, agent loop integration, and evaluation. |
...
| **telephony** | Give Zaya phone capabilities — provision a Twilio number, send/receive SMS/MMS, make calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
