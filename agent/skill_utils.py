...
from zaya_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ZAYA_PLATFORM`` or
            ``ZAYA_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.zaya/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.zaya/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    zaya = metadata.get("zaya") or {}
    if not isinstance(zaya, dict):
        zaya = {}
    return {
        "fallback_for_toolsets": zaya.get("fallback_for_toolsets", []),
        "requires_toolsets": zaya.get("requires_toolsets", []),
        "fallback_for_tools": zaya.get("fallback_for_tools", []),
        "requires_tools": zaya.get("requires_tools", []),
    }
