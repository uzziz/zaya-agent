...
# 5. Symlinks the 'zaya' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Zaya Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink zaya into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up zaya command..."

ZAYA_BIN="$SCRIPT_DIR/venv/bin/zaya"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ZAYA_BIN" "$COMMAND_LINK_DIR/zaya"
echo -e "${GREEN}✓${NC} Symlinked zaya → $COMMAND_LINK_DISPLAY_DIR/zaya"
...
# ============================================================================
# Seed bundled skills into ~/.zaya/skills/
# ============================================================================

ZAYA_SKILLS_DIR="${ZAYA_HOME:-$HOME/.zaya}/skills"
mkdir -p "$ZAYA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.zaya/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ZAYA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  zaya status        # Check configuration"
if is_termux; then
    echo "  zaya gateway       # Run gateway in foreground"
else
    echo "  zaya gateway install # Install gateway service (messaging + cron)"
fi
echo "  zaya cron list     # View scheduled jobs"
echo "  zaya doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m zaya_cli.main setup
fi
