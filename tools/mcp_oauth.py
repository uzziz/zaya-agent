...
This module provides the glue:
    - ``ZayaTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Zaya Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses ZAYA_HOME so each profile gets its own OAuth tokens.
    Layout: ``ZAYA_HOME/mcp-tokens/``
    """
    try:
        from zaya_constants import get_zaya_home
        base = Path(get_zaya_home())
    except ImportError:
        base = Path(os.environ.get("ZAYA_HOME", str(Path.home() / ".zaya")))
...
# ---------------------------------------------------------------------------
# ZayaTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ZayaTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ZAYA_HOME/mcp-tokens/<server_name>.json         -- tokens
        ZAYA_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Zaya.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = ZayaTokenStorage(server_name)
    storage.remove()
...
def build_oauth_auth(
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "Zaya Agent")
    scope = cfg.get("scope")
