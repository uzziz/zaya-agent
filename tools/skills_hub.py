#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Zaya Skills Hub.
...
Used by zaya_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            zaya_meta = metadata.get("zaya", {})
            if isinstance(zaya_meta, dict):
                tags = zaya_meta.get("tags", [])
        if not tags:
...
# ---------------------------------------------------------------------------
# Zaya centralized index source
# ---------------------------------------------------------------------------

ZAYA_INDEX_URL = "https://zaya-agent.nousresearch.com/docs/api/skills-index.json"
ZAYA_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "zaya-index.json"
ZAYA_INDEX_TTL = 6 * 3600  # 6 hours


def _load_zaya_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ZAYA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ZAYA_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ZAYA_INDEX_CACHE_FILE.stat().st_mtime
            if age < ZAYA_INDEX_TTL:
                return json.loads(ZAYA_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ZAYA_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Zaya index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Zaya index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ZAYA_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ZAYA_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ZayaIndexSource(SkillSource):
    """Skill source backed by the centralized Zaya Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "zaya-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "zaya-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ZayaIndexSource(auth=auth), # Centralized index (search + resolved install paths)
