...
it is automatically downloaded from GitHub releases to $ZAYA_HOME/bin/tirith.
...
def _get_zaya_home() -> str:
    """Return the Zaya home directory, respecting ZAYA_HOME env var."""
    return str(get_zaya_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_zaya_home(), ".tirith-install-failed")
...
def _zaya_bin_dir() -> str:
    """Return $ZAYA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_zaya_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ZAYA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_zaya_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ZAYA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ZAYA_HOME/bin/tirith
    ...
    zaya_bin = os.path.join(_zaya_bin_dir(), "tirith")
    if os.path.isfile(zaya_bin) and os.access(zaya_bin, os.X_OK):
        _resolved_path = zaya_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return zaya_bin
