...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    zaya_node_bin = str(zaya_home / "node" / "bin")
    return [zaya_node_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-zaya_…`` the resulting socket path exceeds the
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Zaya-managed
    ...
    # Build an extended search PATH including Zaya-managed Node, macOS
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = zaya_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    screenshots_dir = get_zaya_dir("cache/screenshots", "browser_screenshots")
