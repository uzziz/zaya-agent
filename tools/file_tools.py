...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Zaya internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files.
        from zaya_constants import get_zaya_home as _get_hh
        _zaya_home = _get_hh().resolve()
        _blocked_dirs = [
            _zaya_home / "skills" / ".hub" / "index-cache",
            _zaya_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal Zaya cache file "
