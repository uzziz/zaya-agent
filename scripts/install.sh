...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/zaya-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/zaya-agent.git"
ZAYA_HOME="$HOME/.zaya"
INSTALL_DIR="${ZAYA_INSTALL_DIR:-$ZAYA_HOME/zaya-agent}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.zaya/zaya-agent)"
...
get_zaya_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/zaya" ]; then
        echo "$link_dir/zaya"
    else
        echo "zaya"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/zaya-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$ZAYA_HOME/node/bin/node" ]; then
        export PATH="$ZAYA_HOME/node/bin:$PATH"
        local found_ver=$("$ZAYA_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Zaya-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.zaya/node/..."
    ...
    # Place into ~/.zaya/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$ZAYA_HOME/node"
    mkdir -p "$ZAYA_HOME"
    mv "$extracted_dir" "$ZAYA_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$ZAYA_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$ZAYA_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$ZAYA_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$ZAYA_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$ZAYA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.zaya/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Zaya Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="zaya-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Zaya behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up zaya command..."

    if [ "$USE_VENV" = true ]; then
        ZAYA_BIN="$INSTALL_DIR/venv/bin/zaya"
    else
        ZAYA_BIN="$(which zaya 2>/dev/null || echo "")"
        if [ -z "$ZAYA_BIN" ]; then
            log_warn "zaya not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ZAYA_BIN" ]; then
        log_warn "zaya entry point not found at $ZAYA_BIN"
    ...
    # Create a user-facing shim for the zaya command.
    mkdir -p "$command_link_dir"
    ln -sf "$ZAYA_BIN" "$command_link_dir/zaya"
    log_success "Symlinked zaya → $command_link_display_dir/zaya"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "zaya command ready"
        return 0
    fi
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Zaya Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Zaya Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so zaya works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.zaya directory structure (config at top level, code in subdir)
    mkdir -p "$ZAYA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.zaya/.env (top level, easy to find)
    if [ ! -f "$ZAYA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ZAYA_HOME/.env"
            log_success "Created ~/.zaya/.env from template"
        else
            touch "$ZAYA_HOME/.env"
            log_success "Created ~/.zaya/.env"
        fi
    else
        log_info "~/.zaya/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.zaya/config.yaml (top level, easy to find)
    if [ ! -f "$ZAYA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ZAYA_HOME/config.yaml"
            log_success "Created ~/.zaya/config.yaml from template"
        fi
    else
        log_info "~/.zaya/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$ZAYA_HOME/SOUL.md" ]; then
        cat > "$ZAYA_HOME/SOUL.md" << 'SOUL_EOF'
# Zaya Agent Persona
...
-->
SOUL_EOF
        log_success "Created ~/.zaya/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.zaya/"

    # Seed bundled skills into ~/.zaya/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.zaya/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.zaya/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ZAYA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$ZAYA_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.zaya/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Zaya itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'zaya setup' after install."
        return 0
    fi
    ...
    # Run zaya setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m zaya_cli.main setup < /dev/tty
    else
        python -m zaya_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ZAYA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Zaya to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ZAYA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'zaya whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                ZAYA_CMD="$(get_zaya_command_path)"
                $ZAYA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'zaya whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'zaya gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ZAYA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ZAYA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: zaya gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: zaya gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ZAYA_CMD gateway > "$ZAYA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.zaya/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: zaya gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: zaya gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.zaya/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.zaya/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.zaya/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.zaya/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.zaya/zaya-agent/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}zaya${NC}              Start chatting"
    echo -e "   ${GREEN}zaya setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}zaya config${NC}       View/edit configuration"
    echo -e "   ${GREEN}zaya config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}zaya gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}zaya update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'zaya' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'zaya' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
