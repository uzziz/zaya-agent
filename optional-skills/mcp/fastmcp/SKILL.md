...
- smoke-test a server with the FastMCP CLI before wiring it into Zaya or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Zaya. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.zaya/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.zaya/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Zaya integration, either:

- configure the server in `~/.zaya/config.yaml` using the `native-mcp` skill, or
...
### Zaya cannot see the deployed server

The server-building part may be correct while the Zaya config is not. Load the `native-mcp` skill and configure the server in `~/.zaya/config.yaml`, then restart Zaya.
