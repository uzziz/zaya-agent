---
name: honcho
description: Configure and use Honcho memory with Zaya -- cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation and recall settings.
version: 1.0.0
author: Zaya Agent
license: MIT
metadata:
...
# Honcho Memory for Zaya

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Zaya profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/zaya#running-honcho-locally-with-zaya

### Verify

```bash
zaya honcho status    # shows resolved config, connection test, peer info
```
...
Honcho models conversations as interactions between **peers**. Zaya creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Zaya instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Zaya run |
| `global` | Single session across all directories |

Manual override: `zaya honcho map my-project-name`
...
Each Zaya profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
zaya profile create coder --clone
# creates host block zaya.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `zaya.coder` host block in `honcho.json`
...
```bash
zaya honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Config file: `$ZAYA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `zaya honcho setup`. Ensure `memory.provider: honcho` is in `~/.zaya/config.yaml`.

### Memory not persisting across sessions
Check `zaya honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `zaya profile create <name> --clone`. For existing profiles: `zaya honcho sync`.
...
| Command | Description |
|---------|-------------|
| `zaya honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `zaya honcho status` | Show resolved config, connection test, peer info for active profile |
| `zaya honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `zaya honcho disable` | Disable Honcho for the active profile |
| `zaya honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `zaya honcho peers` | Show peer identities across all profiles |
| `zaya honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `zaya honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `zaya honcho sessions` | List known directory-to-session-name mappings |
| `zaya honcho map <name>` | Map current working directory to a Honcho session name |
| `zaya honcho identity` | Seed AI peer identity or show both peer representations |
| `zaya honcho sync` | Create host blocks for all Zaya profiles that don't have one yet |
| `zaya honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Zaya + Honcho |
| `zaya memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `zaya memory status` | Show active memory provider and config |
| `zaya memory off` | Disable external memory provider |
