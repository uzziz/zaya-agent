...
version: 1.0.0
author: arceus77-7, enhanced by Zaya Agent
license: MIT
metadata:
  zaya:
...
- `tmux` available for stable authenticated sessions during Zaya terminal calls (desktop app flow only)
...
### Service Account (recommended for Zaya)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.zaya/.env` (the skill will prompt for this on first load).
...
## Zaya Execution Pattern (desktop app flow)

Zaya terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/zaya-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/zaya-op.sock"
