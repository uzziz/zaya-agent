...
description: Migrate a user's OpenClaw customization footprint into Zaya Agent. Imports Zaya-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Zaya Agent (Nous Research)
license: MIT
metadata:
  zaya:
    tags: [Migration, OpenClaw, Zaya, Memory, Persona, Import]
    related_skills: [zaya-agent]
---

# OpenClaw -> Zaya Migration

Use this skill when a user wants to move their OpenClaw setup into Zaya Agent with minimal manual cleanup.
...
```bash
zaya claw migrate              # Full interactive migration
...
zaya claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `zaya setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_zaya.py` to:

- import `SOUL.md` into the Zaya home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Zaya memory entries
- merge OpenClaw command approval patterns into Zaya `command_allowlist`
- migrate Zaya-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.zaya/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Zaya workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.zaya/tts/`
- archive non-secret docs that do not have a direct Zaya destination
...
- `scripts/openclaw_to_zaya.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.zaya/skills/migration/openclaw-migration/scripts/openclaw_to_zaya.py`

Do not guess a shorter path like `~/.zaya/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.zaya/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Zaya-compatible secrets, currently:
...
Zaya CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Zaya. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Zaya workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Zaya skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.zaya/skills/migration/openclaw-migration/scripts/openclaw_to_zaya.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.zaya/skills/migration/openclaw-migration/scripts/openclaw_to_zaya.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.zaya/skills/migration/openclaw-migration/scripts/openclaw_to_zaya.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.zaya/skills/migration/openclaw-migration/scripts/openclaw_to_zaya.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.zaya/skills/migration/openclaw-migration/scripts/openclaw_to_zaya.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.zaya/skills/migration/openclaw-migration/scripts/openclaw_to_zaya.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Zaya unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Zaya targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Zaya destination. Unsupported auth blobs must still be reported as skipped.
...
- Zaya persona state imported
- Zaya memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.zaya/skills/openclaw-imports/`
