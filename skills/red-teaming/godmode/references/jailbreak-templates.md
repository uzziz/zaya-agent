...
## 5. GODMODE FAST — Zaya 4 405B (Zero Refusal)

**Model:** `nousresearch/zaya-4-405b`
**Strategy:** Zaya models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Zaya Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.zaya/config.yaml`:
