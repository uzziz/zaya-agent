...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.zaya/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ZAYA_HOME", Path.home() / ".zaya")) / "skills" / "red-teaming" / "godmode"
...
ZAYA_HOME = Path(os.getenv("ZAYA_HOME", Path.home() / ".zaya"))
CONFIG_PATH = ZAYA_HOME / "config.yaml"
PREFILL_PATH = ZAYA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Zaya models — already uncensored, just needs clean prompt
    "zaya": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "zaya" in model_lower or "nous" in model_lower:
        return "zaya"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Zaya config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.zaya/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Zaya for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Zaya for changes to take effect.")
