#!/usr/bin/env python3
"""Google Workspace API CLI for Zaya Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Zaya-facing JSON contract and falls back to the Python client
...
def _gws_binary() -> str | None:
    override = os.getenv("ZAYA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Zaya Agent")
