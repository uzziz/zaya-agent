#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Zaya Agent.
...
try:
    from zaya_constants import display_zaya_home, get_zaya_home
except ModuleNotFoundError:
    ZAYA_AGENT_ROOT = Path(__file__).resolve().parents[4]
    if ZAYA_AGENT_ROOT.exists():
        sys.path.insert(0, str(ZAYA_AGENT_ROOT))
    from zaya_constants import display_zaya_home, get_zaya_home

ZAYA_HOME = get_zaya_home()
TOKEN_PATH = ZAYA_HOME / "google_token.json"
CLIENT_SECRET_PATH = ZAYA_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ZAYA_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Zaya profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Zaya home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_zaya_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Zaya")
