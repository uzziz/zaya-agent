...
- Give it a name like "zaya-agent"
- Select scopes:
...
- Give it a title like "zaya-agent-<machine-name>"
...
elif [ -f ~/.zaya/.env ] && grep -q "^GITHUB_TOKEN=" ~/.zaya/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.zaya/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
