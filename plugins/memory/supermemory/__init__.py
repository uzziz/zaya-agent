...
def _load_supermemory_config(zaya_home: str) -> dict:
    config = _default_config()
    config_path = Path(zaya_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, zaya_home: str) -> None:
    config_path = Path(zaya_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `zaya memory setup`.
        # All other options are documented for $ZAYA_HOME/supermemory.json
    ...
    def save_config(self, values, zaya_home):
        ...
        _save_supermemory_config(sanitized, zaya_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from zaya_constants import get_zaya_home
        self._zaya_home = kwargs.get("zaya_home") or str(get_zaya_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._zaya_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "zaya", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "zaya_memory", "target": target, "type": "explicit_memory"},
