...
Config file: `$ZAYA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `zaya` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `zaya-{identity}` → `zaya-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Zaya profile:

```json
{
  "container_tag": "zaya-{identity}"
}
```

For a profile named `coder`, this resolves to `zaya-coder`. The default profile resolves to `zaya-default`. Without `{identity}`, all profiles share the same container.
