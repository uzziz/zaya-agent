...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/zaya>
...
```bash
zaya honcho setup    # full interactive wizard (cloud or local)
zaya memory setup    # generic picker, also works
```

Or manually:
```bash
zaya config set memory.provider honcho
echo "HONCHO_API_KEY=your-key" >> ~/.zaya/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ZAYA_HOME/honcho.json` | Profile-local (isolated Zaya instances) |
| 2 | `~/.zaya/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Zaya profile: `zaya` (default) or `zaya.<profile>`.
...
Host key derivation: `ZAYA_HONCHO_HOST` env > active profile (`zaya.<profile>`) > `"zaya"`.
...
| `ZAYA_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `zaya honcho setup` | Full interactive setup wizard |
| `zaya honcho status` | Show resolved config for active profile |
| `zaya honcho enable` / `disable` | Toggle Honcho for active profile |
| `zaya honcho mode <mode>` | Change recall or observation mode |
| `zaya honcho peer --user <name>` | Update user peer name |
| `zaya honcho peer --ai <name>` | Update AI peer name |
| `zaya honcho tokens --context <N>` | Set context token budget |
| `zaya honcho tokens --dialectic <N>` | Set dialectic max chars |
| `zaya honcho map <name>` | Map current directory to a session name |
| `zaya honcho sync` | Create host blocks for all Zaya profiles |
