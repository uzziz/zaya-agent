...
Checklist for integrating a new messaging platform into the Zaya gateway.
...
And add it to the `zaya-gateway` composite:

```python
"zaya-gateway": {
    "includes": [..., "zaya-your-platform"]
}
```
...
## 12. Status Display (`zaya_cli/status.py`)
...
## 13. Gateway Setup Wizard (`zaya_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ zaya_cli/ toolsets.py \
  --include="*.py" -l | sort -u
