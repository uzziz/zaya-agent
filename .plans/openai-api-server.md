# OpenAI-Compatible API Server for Zaya Agent
...
SSE streaming. By exposing this endpoint, zaya-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  zaya-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.zaya/.env`
2. Run `zaya gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with zaya-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns zaya-agent as a model) |
...
Authorization: Bearer zaya-api-key-here
...
But zaya-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "zaya-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured zaya-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of zaya-agent's core prompt
...
Once implemented, zaya-agent works as a drop-in backend for:
